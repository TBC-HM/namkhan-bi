...
// Lowercased, ignores spaces and punctuation.
...
export function matchSourceToContract(
  ...
  for (const c of contracts) {
    const n = normalize(c.partner_short_name);
    if (!n) continue;
    if (src.includes(n) || n.includes(src)) {
      return { contract_id: c.contract_id, partner_short_name: c.partner_short_name };
    }
  }
