// lib/emailAgents/engine.ts
// v5.0 (2026-07-26, Newsletter Writer Team v1 · Layer 0) · MOVED from
// app/api/marketing/newsletter/propose-one/engine.ts (which now re-exports this
// module — REUSE-FIRST: one writer implementation, two route surfaces).
// Layer 0 changes only — behavior-preserving extraction plus:
//   - property_id PARAMETERIZED end-to-end (NAMKHAN_ID hardcode removed;
//     DEFAULT_PROPERTY_ID=260955 retained solely for legacy callers that pass
//     no property_id — newsletter-v2 routes always pass it explicitly).
//   - refreshLiveContext(property_id, …) exported: loadContext + pace state +
//     per-surface ok/fail + stale detection (v3 amendment: agents MUST NOT
//     produce output on stale context — enforced when body.strict_context).
...
// Legacy default only — v2 routes pass property_id explicitly (URL LAW).
export const DEFAULT_PROPERTY_ID = 260955;
...
export type ProposeBody = {
  ...
  concept?: string;
  campaign_id?: string;
  // Layer 0 (v5): stale-context guard — when true the writer MUST NOT produce
  // output if any critical grounding surface is broken/empty (v3 amendment).
  // newsletter-v2 routes always set this; legacy routes keep soft behavior
  // until the A11 UI rewire cuts them over.
...
async function loadContext(sb: ReturnType<typeof getSupabaseAdmin>, pid: number, group_slug: string | null) {
  const [rulesR, linksR, retreatsR, activitiesR, sendersR, goalsR, policyR, groupR, realityR, roomsR, facilitiesR, transportR, cruisesR, learningsR] = await Promise.all([
    ...
    sb.from('v_property_retreats').select('name:display_name, short_description:short_pitch, focus_notes:ideal_for').eq('property_id', pid).eq('is_active', true).limit(10),
    sb.from('v_activities_catalog').select('activity_name:name, short_description:description, price_incl_taxes_usd:price_amount').eq('property_id', pid).eq('is_active', true).order('display_order', { ascending: true }).limit(15),
    ...
    sb.from('v_boat_cruises').select('name, cruise_type, route_from, route_to, duration_min').eq('property_id', pid).eq('is_active', true).order('display_order', { ascending: true }).limit(8),
...
// ── Layer 0 (v5): pace state + live-context refresh + stale guard ────────────

export type PaceState = {
  ci_month: string;
  otb_roomnights: number | null; otb_revenue: number | null;
  stly_roomnights: number | null; stly_revenue: number | null;
  roomnights_delta: number | null; revenue_delta: number | null;
};

// Latest on-the-books pace for the stay month the email targets (mv_pace_otb).
// target_date absent → nearest current/future stay month. Never fatal.
export async function loadPaceState(sb: ReturnType<typeof getSupabaseAdmin>, pid: number, target_date?: string | null): Promise<PaceState | null> {
  try {
    const m = target_date && /^\d{4}-\d{2}/.test(String(target_date)) ? String(target_date).slice(0, 7) + '-01' : new Date().toISOString().slice(0, 7) + '-01';
    const { data, error } = await sb.from('mv_pace_otb')
      .select('ci_month, otb_roomnights, otb_revenue, stly_roomnights, stly_revenue, roomnights_delta, revenue_delta')
      .eq('property_id', pid).gte('ci_month', m)
      .order('ci_month', { ascending: true }).limit(1);
    if (error) return null;
    const row = (data as PaceState[] | null)?.[0];
    return row ? { ...row, ci_month: String(row.ci_month) } : null;
  } catch { return null; }
}

// Critical-surface staleness (v3 amendment). 'rules' is excluded: that surface
// is knowingly disabled pending PBS arbitration (v4.3 note above). 'learnings'
// and 'policy'/'group' (null when no group_slug) are excluded: empty is normal.
export function staleSurfaces(ctx: Awaited<ReturnType<typeof loadContext>>): string[] {
  const stale: string[] = [];
  const IGNORE = new Set(['rules', 'learnings']);
  for (const [k, ok] of Object.entries(ctx.surfaces)) if (!ok && !IGNORE.has(k)) stale.push(`${k}:query_failed`);
  if (!ctx.sender) stale.push('sender:empty');
  if (ctx.links.length === 0) stale.push('links:empty');
  if (ctx.rooms.length === 0) stale.push('rooms:empty');
  if (ctx.facilities.length === 0) stale.push('facilities:empty');
  if (ctx.retreats.length === 0) stale.push('retreats:empty');
  if (ctx.activities.length === 0) stale.push('activities:empty');
  if (!ctx.reality) stale.push('reality_profile:empty');
  return Array.from(new Set(stale));
}

export type LiveContext = { ctx: Awaited<ReturnType<typeof loadContext>>; pace: PaceState | null; stale: string[] };

// MANDATORY Layer 0 refresh (v3 amendment): every agent call pulls fresh prod
// state — sender chrome, retreats, rooms, facilities, activities, groups,
// policy, link catalog, reality profile, learnings, pace — before reasoning.
export async function refreshLiveContext(pid: number, group_slug: string | null, target_date?: string | null): Promise<LiveContext> {
  const sb = getSupabaseAdmin();
  const [ctx, pace] = await Promise.all([
    loadContext(sb, pid, group_slug),
    loadPaceState(sb, pid, target_date ?? null),
  ]);
  return { ctx, pace, stale: staleSurfaces(ctx) };
}
...
export async function fallbackPhotoPick(sb: ReturnType<typeof getSupabaseAdmin>, pid: number): Promise<PhotoPick[]> {
...
async function loadDeepDive(sb: ReturnType<typeof getSupabaseAdmin>, pid: number, kws: string[]): Promise<DeepDive> {
...
function assembleUserPrompt(
  ...
  if (pace) {
    parts.push('');
    parts.push(`### PACE STATE (on-the-books vs same time last year · stay month ${String(pace.ci_month).slice(0, 7)})`);
    parts.push(`room nights OTB: ${pace.otb_roomnights ?? '—'} (STLY ${pace.stly_roomnights ?? '—'} · delta ${pace.roomnights_delta ?? '—'})`);
    parts.push(`revenue OTB: ${pace.otb_revenue ?? '—'} (STLY ${pace.stly_revenue ?? '—'} · delta ${pace.revenue_delta ?? '—'})`);
    parts.push('Use ONLY to calibrate the angle (urgency, season, timing). NEVER quote these numbers or say "quiet/busy" in guest-facing prose.');
  }

  if (ctx.learnings.length > 0) {
    parts.push('');
    parts.push('### LEARNED PREFERENCES (Mira memory · distilled from owner edits of past emails · obey)');
    for (const l of ctx.learnings) {
      const rule = (l.learned_rule || l.edit_summary || '').trim();
      if (rule) parts.push(`- ${rule}${l.field ? ` [${l.field}]` : ''}`);
    }
  }
...
export async function proposeOne(body: ProposeBody): Promise<NextResponse> {
  ...
  // Layer 0 (v5) property resolution: explicit body.property_id wins; a
  // campaign row must agree with it (mismatch = not found); legacy callers
  // with neither get DEFAULT_PROPERTY_ID until the A11 cutover.
  const requestedPid = (Number.isFinite(Number(body.property_id)) && Number(body.property_id) > 0)
  ...
  if (campaign_id) {
    ...
    if (requestedPid !== null && campaign.property_id !== requestedPid) return NextResponse.json({ ok: false, error: 'campaign_not_found' }, { status: 404 });
  ...
  const pid = campaign?.property_id ?? requestedPid ?? DEFAULT_PROPERTY_ID;
  ...
  // Layer 0 (v5) MANDATORY live refresh: context + pace pulled fresh from prod
  // on every call (rules, links, reality profile, rooms, facilities, transport,
  // boat, retreats, activities, goals, policy, group voice, learnings, pace).
  const live = await refreshLiveContext(pid, group_slug, body.target_date ?? null);
  const ctx = live.ctx;

  // Stale-context guard (v3 amendment): strict callers get NO output on a
  // broken/empty grounding surface — surfaced as 424 stale_context.
  if (body.strict_context && live.stale.length > 0) {
    return NextResponse.json({ ok: false, error: 'stale_context', stale: live.stale, surfaces: ctx.surfaces }, { status: 424 });
  }
  ...
  if (envelopePhotoCount === 0) {
    const tFb = Date.now();
    fallbackPhotos = await fallbackPhotoPick(sb, pid);
  ...
  const deep = await loadDeepDive(sb, pid, conceptKeywords);
  ...
  const userPrompt = assembleUserPrompt(body, ctx, envelope, fallbackPhotos, concept, deep, keyDates, null, live.pace);
  ...
  if (!isRefine && veda.score < 60 && veda.critique) {
    ...
    try {
      const retryPrompt = assembleUserPrompt(body, ctx, envelope, fallbackPhotos, concept, deep, keyDates, veda.critique, live.pace);
