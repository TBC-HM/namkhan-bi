...
// app/operations/_components/RomMailPanel.tsx
// Operations HoD landing — Reservations Operations Manager mail feed.
// Server-side Gmail filter: from:rom@thenamkhan.com -label:HOD-DISMISSED (shared label with Revenue).
...
export default function RomMailPanel() {
  ...
  const load = useCallback(async () => {
    try {
      const r = await fetch('/api/hod/operations/mails?max=30', { cache: 'no-store' });
      if (r.status === 401) {
  ...
  const dismiss = useCallback(async (id: string) => {
    ...
    try {
      const r = await fetch('/api/hod/operations/mails/' + id + '/dismiss', { method: 'POST' });
      const j = await r.json();
      if (j?.ok) {
    ...
    <div style={{ background: BG, border: '1px solid ' + HAIRLINE, borderRadius: 6, width: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', padding: '10px 14px', borderBottom: '1px solid ' + HAIRLINE, background: BG_INSET }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: INK }}>Reservations Operations Manager</div>
          <div style={{ fontSize: 11, color: MUTED }}>rom@thenamkhan.com · last 30 messages · dismissed items hidden</div>
        </div>
      ...
      {rows !== null && error === 'auth' && (
        <div style={{ padding: 16, fontSize: 12, color: INK }}>
          ...
          <a href="/settings/gmail" style={{ color: UNREAD_DOT, textDecoration: 'none', fontWeight: 600 }}>
            Connect your Gmail to see messages &rarr;
          </a>
        </div>
      )}
      ...
      {rows !== null && !error && rows.length === 0 && (
        <div style={{ padding: 16, fontSize: 12, color: MUTED, fontStyle: 'italic' }}>
          No new messages from Reservations Operations Manager
        </div>
      )}
