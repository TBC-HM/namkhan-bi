// app/operations/spa/_cockpit/LegacySpaView.tsx
// PBS 2026-08-30 · The previous Spa overview page, preserved verbatim.
//
// It opened on a USALI rollup and a QB GL breakdown — a controller's view a spa
// manager never opened — so the Overview route is now the manager cockpit and
// this moved behind the Ledger tab. NOTHING was deleted: every container, query
// and figure below is exactly as it was. The only edits are mechanical: the
// DashboardPage/SpaSubnav wrapper is removed (the Ledger route supplies its own)
// and the hardcoded tenant fallback is removed, since defaulting the property
// would serve Namkhan's spa numbers to any other tenant (L22).
...
import { Container, KpiTile, type KpiTileProps, type DashboardTab } from '@/app/(cockpit)/_design';
...
export default async function LegacySpaView({ searchParams, propertyId }: Props) {
  // L22: no tenant default. The caller always supplies this.
