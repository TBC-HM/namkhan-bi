...
export function StockOnHandTable({ rows }: { rows: StockOnHandRow[] }) {
  const cols: Column<StockOnHandRow>[] = [
    ...
    { key: 'sold_ytd', header: 'Sold YTD', numeric: true, width: '90px',
      render: (r) => <span style={mono}>{fmtQty(r.sold_ytd)}</span>, sortValue: (r) => r.sold_ytd ?? -1 },
    { key: 'sold_30d', header: 'Sold 30d', numeric: true, width: '90px',
      render: (r) => <span style={mono}>{fmtQty(r.sold_30d)}</span>, sortValue: (r) => r.sold_30d ?? -1 },
