...
//
// Catalog-Admin upload modal — accepts a CSV (typically a Cloudbeds Items
// export) and bulk-loads rows into inv.items via /api/operations/inventory/items.
//
// Required columns (case-insensitive, exact header text shown here):
//   sku             — unique within property
//   item_name       — display name
//   category_code   — must match inv.categories.code (FB_FOOD, LINEN, etc.)
//
// Optional columns:
//   unit_code       — defaults to "ea" if blank (must match inv.units.code)
//   description, last_unit_cost_usd, gl_account_code, is_perishable,
//   shelf_life_days, reorder_point, reorder_quantity, notes
//
// On submit the file is POSTed as JSON (parsed client-side) so we can show
// per-row success/failure inline before any DB write succeeds.
...
// Tiny CSV parser — handles quoted cells with embedded commas + escaped quotes.
...
export default function UploadProductsButton() {
  ...
  async function onFile(list: FileList | null) {
    ...
    try {
      ...
      const need = ['sku', 'item_name', 'category_code'];
  ...
  async function uploadAll() {
    ...
    setBusy(true);
    // Mark all as inserting upfront so the UI feels live during the round-trip
    setRows(prev => prev.map(r => r.status === 'queued' ? { ...r, status: 'inserting' } : r));
    try {
      const payload = rows
        .filter(r => r.status === 'inserting' || r.status === 'queued')
        .map(r => r.raw);
      const resp = await fetch('/api/operations/inventory/items', {
        method: 'POST',
      ...
      if (!resp.ok) {
        ...
        const bySku = new Map<string, ApiResultRow>();
        results.forEach(rr => bySku.set(rr.sku, rr));
        setRows(prev => prev.map(r => {
          const hit = bySku.get(r.sku);
          if (!hit) return { ...r, status: 'error', message: 'No server response for SKU' };
          ...
          <div className="w-full max-w-3xl rounded-md bg-white p-6 text-sm text-stone-800 shadow-xl">
            <div className="mb-4 flex items-baseline justify-between">
              <h2 className="font-serif text-2xl text-stone-900">
                Upload <em className="italic">products</em>
              </h2>
              <button
                type="button"
                disabled={busy}
            ...
            <div className="mb-4 rounded-sm border border-stone-200 bg-stone-50 p-3 text-xs text-stone-600">
              ...
              <div className="font-mono">sku, item_name, category_code</div>
              <div className="mt-1 font-mono text-[10px] uppercase tracking-[0.16em] text-stone-500">Optional</div>
              <div className="font-mono">unit_code (default: ea), description, last_unit_cost_usd, gl_account_code, is_perishable, shelf_life_days, reorder_point, reorder_quantity, notes</div>
              <div className="mt-2 text-stone-500">Existing SKUs will be <strong>updated</strong> (upsert by sku). Unknown category_code will be flagged as error per row.</div>
            </div>
            ...
            {rows.length > 0 && (
              <div className="mb-3 max-h-72 overflow-y-auto rounded-sm border border-stone-200">
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {rows.map(r => (
                      <tr key={`${r.rowIndex}-${r.sku}`} className="border-t border-stone-100">
                        ...
                        <td className="px-2 py-1 font-mono">{r.category_code || '—'}</td>
                        <td className="px-2 py-1 font-mono">{r.unit_code}</td>
