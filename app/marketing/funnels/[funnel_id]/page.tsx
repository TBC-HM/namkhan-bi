// app/marketing/funnels/[funnel_id]/page.tsx
// PBS 2026-07-05: Funnel step viewer (read-only v1) — server-rendered, no client editor yet.
...
type DetailRow = {
  ...
  blocks_json: unknown; hero_image_url: string | null; click_tag_map: Record<string, unknown> | null;
  step_active: boolean | null; step_sends: number | null;
};

const fmtDelay = (d: number | null) => d == null ? '—' : (d === 0 ? 'immediate' : d === 1 ? '+1 day' : `+${d} days`);

export default async function FunnelDetailPage({ params }: { params: { funnel_id: string } }) {
  ...
  return (
    <div style={{ background:'#FFFFFF', minHeight:'100vh' }}>
      <DashboardPage
        title={`Marketing · Funnel · ${head.name}`}
        subtitle={`${steps.length} step${steps.length===1?'':'s'} · state: ${head.status}${head.auto_enroll ? ' · auto-enrolls new subscribers' : ''}`}
        tabs={tabs}
      >
        <div style={{ gridColumn:'1 / -1', display:'flex', justifyContent:'space-between', alignItems:'center', gap:8, flexWrap:'wrap' }}>
          <Link href="/marketing/funnels" style={{ fontSize:12, color:'#084838', textDecoration:'none', fontWeight:600 }}>← Back to funnels</Link>
          <div style={{ display:'flex', gap:8 }}>
            <Link href={`/marketing/funnels/${head.funnel_id}/preview`} style={btnLight}>Preview</Link>
            <Link href={`/marketing/funnels/${head.funnel_id}/enroll`}  style={btnGreen}>Enroll subscribers</Link>
          </div>
        </div>

        <div style={{ gridColumn:'1 / -1', border:'1px solid '+HAIR, borderRadius:6, background:'#FFFFFF', padding:16 }}>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(160px, 1fr))', gap:12 }}>
            <MetaCell label="Key"            value={head.funnel_key} />
            <MetaCell label="Status"         value={head.status} />
            <MetaCell label="Target segment" value={head.target_tag_label ?? '—'} />
            <MetaCell label="Steps"          value={String(steps.length)} />
            <MetaCell label="Auto-enroll"    value={head.auto_enroll ? 'yes' : 'no'} />
          </div>
          {head.description && (
        ...
        <div style={{ gridColumn:'1 / -1' }}>
          ...
          {steps.length === 0 ? (
            <div style={{ padding:'20px 24px', fontSize:12, color:INK_M, background:'#FFFFFF', border:'1px solid '+HAIR, borderRadius:6, textAlign:'center' }}>
              No steps yet. Use <Link href="/marketing/funnels/ai-propose" style={{ color:'#084838' }}>AI propose funnel</Link> or add steps manually (editor coming next).
            </div>
          ) : (
...
function MetaCell({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
      ...
      <div style={{ fontSize:12, color:'#1B1B1B', fontVariantNumeric:'tabular-nums' }}>{value}</div>
    </div>
  );
}
