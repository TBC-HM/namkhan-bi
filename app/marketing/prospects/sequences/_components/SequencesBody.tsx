// app/marketing/prospects/sequences/_components/SequencesBody.tsx
// PBS 2026-07-21: extracted from ../page.tsx so both the legacy
// /marketing/prospects/sequences path (now a redirect) and the new home
// /guest/newsletters/sequences can render the same list without duplicating
// code. Takes tabs + optional subStrip so each host page picks its own chrome.
//
// Original body was authored 2026-07-05 as the newsletter-style sequences list.
...
export default function SequencesBody({ title, subtitle, tabs, rows, error, backHref, backLabel, subStrip }: Props) {
    ...
    <div style={{ background:'#FFFFFF', minHeight:'100vh' }}>
      <DashboardPage title={title} subtitle={subtitle} tabs={tabs}>
        ...
        <div style={{ gridColumn:'1 / -1', display:'flex', justifyContent:'space-between', alignItems:'center', gap:8, flexWrap:'wrap' }}>
          <TenantLink href={backHref} style={{ fontSize:12, color:'#084838', textDecoration:'none', fontWeight:600 }}>
            ← {backLabel}
          </TenantLink>
...
function Section({ title, rows, kind }: { title: string; rows: SequenceRow[]; kind: 'draft'|'live'|'halted' }) {
