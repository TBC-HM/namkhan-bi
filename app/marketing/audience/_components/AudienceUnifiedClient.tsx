...
//
// 2026-07-21 pm · Per-row + bulk Newsletter / Sequence action buttons.
//   • Per-row: forest-green outline buttons (Newsletter opens /guest/newsletters
//     prefill; Sequence opens /marketing/prospects/sequences prefill).
//     Prospect rows only get Sequence (no newsletter path for un-opted-in prospects).
//   • Bulk bar: "Send Newsletter" + "Add to Sequence" (comma-separated recipients),
//     plus a "Select all filtered (N)" convenience toggle next to the search field.
...
export default function AudienceUnifiedClient({
  ...
  const bulkAddToSequence = () => {
    if (selectedEmails.length === 0) { setMsg('Select rows first.'); return; }
    const qs = new URLSearchParams({ recipients: selectedEmails.join(',') }).toString();
    window.open(`/marketing/prospects/sequences/new?${qs}`, '_blank', 'noopener,noreferrer');
  };
