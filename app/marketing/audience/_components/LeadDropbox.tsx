...
// PBS 2026-07-23 · Compact upload-new-leads dropbox above KPI tiles.
// #334 · Optional custom source name — when set, overrides "upload:{filename}"
// as the staging.source_file so the eventual newsletter_subscribers.source
// promotion inherits the human-readable label ("IHF Bangkok 2026 booth" etc).
...
export default function LeadDropbox() {
  ...
  const upload = useCallback(async (file: File) => {
    ...
    if (trimmed) form.append("source_name", trimmed);
    try {
      ...
      setState("done");
      const src = j.source_file ? ` · source=${j.source_file}` : "";
      setMsg(`Parsed ${j.rows_found ?? 0} rows · ${j.rows_upserted ?? 0} staged for review.${src}`);
    } catch (e) {
      setState("error");
  ...
  return (
    ...
    >
      ...
      <input
        ...
        style={{
          padding: "6px 10px", border: `1px solid ${HAIR}`, background: WHITE,
          color: INK, borderRadius: 3, fontSize: 12, width: 200,
        }}
      />
      <button
