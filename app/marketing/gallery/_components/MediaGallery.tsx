// app/marketing/gallery/_components/MediaGallery.tsx — same as media/ version
'use client';
...
export default function MediaGallery({ rows }: Props) {
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <div style={{ padding:40, textAlign:'center', color:INK_M, background:'#FFFFFF', border:'1px solid '+HAIR, borderRadius:4 }}>No assets match your filter.</div>
      ) : (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:12 }}>
          {filtered.slice(0, 300).map((r) => (
            <button key={r.asset_id} onClick={() => r.public_url && copyUrl(r.public_url, r.original_filename)}
              ...
              style={{ background:'#FFFFFF', border:'1px solid '+HAIR, borderRadius:4, padding:0, cursor: r.public_url ? 'pointer' : 'default', overflow:'hidden', textAlign:'left', display:'flex', flexDirection:'column' }}>
              {r.public_url ? (
              ...
              <div style={{ padding:'6px 8px', fontSize:10, color:INK, borderTop:'1px solid '+HAIR }}>
                ...
                {copied === r.original_filename && (<div style={{ marginTop:4, color:NK_GREEN, fontWeight:600 }}>✓ URL copied</div>)}
              </div>
            </button>
          ))}
        </div>
