...
// app/cockpit-v2/activity/ActivityView.tsx
// Renders the unified activity timeline + a 30s poll that refreshes from
// /api/cockpit-v2/activity. Filter pills toggle each source on/off.
//
...
export function ActivityView({
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        setPending(true);
        const res = await fetch('/api/cockpit-v2/activity?limit=200', {
          cache: 'no-store',
        });
        if (!res.ok) return;
