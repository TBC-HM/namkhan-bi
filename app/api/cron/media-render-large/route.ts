// app/api/cron/media-render-large/route.ts
...
//
// Lives under /api/cron/* (middleware bypass — house pattern for headless calls);
// auth enforced in-route: x-upload-key = vault MEDIA_UPLOAD_KEY, or the cron
// shared secret via x-cron-secret (CRON_SHARED_SECRET / legacy CRON_SECRET).
// Body: { asset_id: "<uuid>" }
...
export async function POST(req: NextRequest) {
  ...
  // Guard: vault MEDIA_UPLOAD_KEY (same key media-render-web v2 upload mode uses)
  // or the cron shared secret (house pattern for /api/cron/* routes).
  const givenUpload = req.headers.get('x-upload-key') ?? '';
  ...
  const uploadOk = !!secret && typeof secret === 'string' && givenUpload.length >= 20 && givenUpload === secret;
  const cronOk = cronSecret.length >= 10 && givenCron === cronSecret;
  if (!uploadOk && !cronOk) {
