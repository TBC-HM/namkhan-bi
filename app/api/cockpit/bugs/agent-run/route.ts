...
// PBS 2026-07-17 — Bug-agent orchestrator · Phase B (real AI + shipping).
//
// POST /api/cockpit/bugs/agent-run
//   body: { bug_ids?: number[]; mode?: 'one'|'drain'; max?: number; triggered_by?: string }
// GET  /api/cockpit/bugs/agent-run           → last 50 runs
// GET  /api/cockpit/bugs/agent-run?run_id=X  → single run detail
//
// Pipeline per bug:
//   1. PLANNING  — Anthropic call with bug body + page_url + fetched candidate
//                  file(s) → returns {plan_md, patches[], skip_reason?}
//   2. REVIEWING — 2nd Anthropic call scoring the plan → approve|reject|human
//   3. SHIPPING  — creates bots/bug-{id} branch off main, pushes patches via
//                  gh api, opens PR to main. NEVER auto-merges (v2 policy).
//   4. VERIFYING — polls Vercel status until success/failure (or 3 min).
//                  Curls page_url · checks 200/307 + no "Application error".
//   5. CLOSING   — updates bug status='in_progress' (awaiting merge) or
//                  'blocked' with reason.
//
// Cost cap: $2 per invocation. Model = claude-sonnet-4-6.
...
const MAX_FILE_BYTES = 40_000; // truncate very long files to save tokens
...
interface FilePatch { path: string; new_content: string; reasoning: string }
interface PlannerResult { plan_md: string; patches: FilePatch[]; skip_reason?: string; cost_usd: number }
interface ReviewerResult { verdict: 'approve' | 'reject' | 'needs_human'; notes: string; cost_usd: number }
...
async function ghGetCheckStatus(sha: string): Promise<{ ci_ok: boolean | null; note: string }> {
  ...
  const status = statusR.ok ? await statusR.json() as { state: string; statuses: Array<{ context: string; state: string }> } : { state: 'pending', statuses: [] };
...
function guessCandidateFiles(bug: { page_url: string | null; body: string | null }): string[] {
  ...
  try {
    ...
    if (parts[0] === 'h' && parts[1] && /^\d+$/.test(parts[1])) parts.splice(0, 2, ''); // skip /h/{pid}
    const seg = parts.filter(Boolean);
    if (seg.length > 0) {
      files.push(`app/${seg.join('/')}/page.tsx`);
      // Common client component name pattern for /holding/bugs, /marketing/media, etc.
  ...
  if (body.includes('sub menu') || body.includes('submenu') || body.includes('nav')) {
...
const REVIEWER_SYSTEM = [
  'You are a strict code reviewer for a Next.js hotel BI app. Given a bug + a proposed set of file patches, return ONLY a JSON object:',
  ...
  'NEEDS_HUMAN if: patches attempt a page rewrite, touch >3 files, or the bug is ambiguous.',
...
async function shipPatches(bug: { id: number; body: string | null }, plan: PlannerResult): Promise<{ branch: string; commit_sha: string; pr_number: number; pr_url: string }> {
  ...
  for (const patch of plan.patches) {
    // Fetch existing sha for the file on the NEW branch (needed for update).
...
async function verifyDeploy(commitSha: string, pageUrl: string | null): Promise<{ ci_ok: boolean | null; curl_status: number | null; curl_body_ok: boolean | null; note: string }> {
  // Poll CI up to ~90s (feature-branch CI runs typically finish in 60-90s).
  ...
  // Curl the affected page URL (only if it's an app.namkhan-bi.vercel.app URL and CI passed).
...
// ---------- Per-bug run ----------
async function runOneBug(bug: { id: number; body: string | null; page_url: string | null; dept_slug: string | null }, triggeredBy: string, budgetLeftUsd: number): Promise<{ bug_id: number; run_id?: number; ok: boolean; phase?: string; error?: string; cost_usd?: number }> {
  const sb = getSupabaseAdmin();
...
export async function POST(req: Request) {
  const body = (await req.json().catch(() => ({}))) as { bug_ids?: number[]; mode?: 'one' | 'drain'; max?: number; triggered_by?: string };
  const mode = body.mode ?? 'one';
  const max = Math.min(Math.max(1, body.max ?? 1), 3); // v2 conservative: max 3 per invocation (Anthropic latency)
  ...
  let q = sb.schema('cockpit').from('exec_bugs')
    ...
    .eq('agent_skip', false);
  if (body.bug_ids && body.bug_ids.length > 0) q = q.in('id', body.bug_ids);
  ...
  if (rows.length === 0) return NextResponse.json({ ok: true, message: 'No eligible bugs.', processed: [] });

  const results: Array<{ bug_id: number; run_id?: number; ok: boolean; phase?: string; error?: string; cost_usd?: number }> = [];
  let totalCost = 0;
  for (const bug of rows) {
    if (totalCost >= COST_CAP_USD) {
      results.push({ bug_id: bug.id, ok: false, error: `cost_cap_reached ($${totalCost.toFixed(2)} of $${COST_CAP_USD})` });
      break;
    }
    const r = await runOneBug(bug, triggeredBy, COST_CAP_USD - totalCost);
    results.push(r);
    totalCost += r.cost_usd ?? 0;
  }

  return NextResponse.json({ ok: true, phase: 'B', mode, cost_usd: totalCost, processed: results });
}
