...
export async function GET(req: NextRequest) {
  ...
  const pid = req.nextUrl.searchParams.get('property_id');
  const sb = getSupabaseAdmin();
  let q = sb.from('v_dataroom_rooms').select('*').order('created_at', { ascending: false });
  if (level === 'holding') q = q.eq('owner_level', 'holding');
  else if (pid) q = q.eq('owner_level', 'property').eq('property_id', Number(pid));
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await sb.rpc('fn_dataroom_create_room', {
    p_owner_level: level,
    p_property_id: level === 'holding' ? null : Number(body.property_id),
    p_slug: slug,
    p_name: name,
