...
import { brainAsk, SCOPE_CFG, type BrainScope } from '@/lib/brain/ask-core';
...
export async function POST(req: NextRequest) {
  ...
  // BRAIN v6: department-scoped windows ("universal chatter with borders").
  // Each scope pins corpus kinds + max tier in SQL; 'all' = owner surface.
  const scope: BrainScope = (['sops', 'marketing', 'revenue', 'operations', 'admin'] as const)
    .includes(body.scope as never) ? (body.scope as BrainScope) : 'all';
  const tier = scope === 'all' ? V1_TIER : SCOPE_CFG[scope].tier;
