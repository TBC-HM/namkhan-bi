...
export async function POST(req: Request) {
  ...
  const propertyId = await requirePropertyAccess(req as any, b.property_id);
  const sb = getSupabaseAdmin();
  const op = String(b.op || '');
  try {
    if (op === 'create') {
      const p: any = { property_id: propertyId, platform: b.platform || 'instagram', caption: b.caption || '', created_by: 'pbs' };
      ...
      if (b.event_id) await callEdge('social-compose', { property_id: propertyId, event_id: String(b.event_id) });
    } else {
