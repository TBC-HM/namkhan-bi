...
//
// Walks every ACTIVE row in marketing.user_gmail_connections (or a single account
// if account_email is passed) and extracts every sender/recipient email address
// from message headers (headers only — no bodies, no attachments), deduping into
...
//
// POST body: { account_email?: string, max_messages?: number }
//   - account_email: extract just this connected mailbox (else all active).
//   - max_messages: cap per-account scan (default 5000, cron default 2000).
//
// Response: { ok:true, runs:[{ account_email, messages_scanned, new_contacts, updated_contacts, run_id }] }
...
async function extractForConnection(row: ConnRow, maxMessages: number): Promise<RunResult> {
...
export async function POST(req: Request) {
  ...
  const q = admin
    ...
    .eq('active', true);
  const conns = body.account_email
    ? await q.eq('gmail_address', body.account_email)
    : await q;
  if (conns.error) {
    return NextResponse.json({ ok: false, error: 'conn_list_' + conns.error.message }, { status: 500 });
  }
  const rows = (conns.data ?? []) as ConnRow[];
  if (rows.length === 0) {
    return NextResponse.json({ ok: false, error: 'no_active_connections' + (body.account_email ? '_for_' + body.account_email : '') }, { status: 404 });
  }
...
export async function GET() {
  return NextResponse.json({
    ok: true,
    hint: 'POST to trigger. Header x-cron-secret or admin session required. Body: { account_email?, max_messages? }',
  });
}
