...
// POST {page_id, property_id} → create version via fn_website_create_version
...
export async function POST(req: NextRequest) {
  ...
  const { page_id, property_id } = body;
  if (!page_id || !property_id) {
    return NextResponse.json({ error: 'page_id and property_id required' }, { status: 400 });
  }

  const sb = getSupabaseAdmin();
  const { data, error } = await sb.rpc('fn_website_create_version', {
    page_id,
    property_id,
    created_by: null,
    restore_note: 'UI snapshot',
  });
