...
// PBS 2026-05-28 — ADR-128 honest-leakage swap + dynamic action descriptors + single-row 8-up
// 8 YTD headline tiles bound to v_leakage_top_summary_v2.
// Footnotes are PULLED LIVE from v_leakage_action_pointers (MV, ~3ms): each
// tile surfaces the single largest breach (month + room + €). size='sm' +
// 8-column grid keeps all 8 in one row on standard laptop+ widths.
...
export default async function LeakageYtdTiles({ propertyId }: Props) {
  ...
  const fnRevenue = `${yr} YTD realised`;
  const fnAddressable = pt?.top_addressable_channel && pt?.top_addressable_eur
    ? `top: ${pt.top_addressable_channel} ${fmtCcy(pt.top_addressable_eur, ccy)}`
    : 'bedbank + OTA + email + phone';
  const fnAddressablePct = `${fmtCcy(Number(top?.honest_leakage_ytd ?? 0), ccy)} of revenue · honest`;
  const fnBedbank = pt?.bedbank_top_partner && pt?.bedbank_top_room
    ? `${pt.bedbank_top_partner} · ${pt.bedbank_top_room}`
    : 'sold below contracted floor';
  const fnOta = pt?.ota_top_month && pt?.ota_top_room
    ? `worst ${fmtMonth(pt.ota_top_month)} · ${pt.ota_top_room} ${fmtCcy(pt.ota_top_eur, ccy)}`
    : `${Math.round(Number(rd?.ota_parity_breach_rn ?? 0)).toLocaleString()} RN below direct`;
  const fnSpread = netSpread < 0
    ? (pt?.spread_top_month && pt?.spread_top_room
        ? `win ${fmtMonth(pt.spread_top_month)} · ${pt.spread_top_room} ${fmtCcy(pt.spread_top_eur, ccy)}`
        : 'direct cheaper than OTA')
    : netSpread > 0
      ? 'website undercutting OTA'
      : 'direct ≈ OTA';
  const fnEmail = pt?.email_top_category
    ? `${pt.email_top_category} · ${(pt.email_top_rn ?? 0).toLocaleString()} RN @ ${fmtCcy(pt.email_top_adr, ccy)} vs ${fmtCcy(pt.email_top_baseline, ccy)}`
    : 'transient-only, occ-weighted';
  const fnPhone = pt?.phone_top_month && pt?.phone_top_room
    ...
    : `${Math.round(Number(rd?.phone_breach_rn ?? 0)).toLocaleString()} RN below direct`;

  const tiles = [
    { label: 'Revenue YTD', value: Math.round(Number(top?.revenue_ytd ?? 0)), currency: ccy, footnote: fnRevenue, status: 'green' as const, size: 'sm' as const },
    { label: 'Addressable Leakage', value: Math.round(Number(top?.honest_leakage_ytd ?? 0)), currency: ccy, footnote: fnAddressable, status: sevByPct(Number(top?.honest_leakage_pct ?? 0)), size: 'sm' as const },
    { label: 'Leakage %', value: `${Number(top?.honest_leakage_pct ?? 0).toFixed(1)}%`, footnote: fnAddressablePct, status: sevByPct(Number(top?.honest_leakage_pct ?? 0)), size: 'sm' as const },
    { label: 'Bedbank (below floor)', value: Math.round(Number(top?.bedbank_leakage_ytd ?? 0)), currency: ccy, footnote: fnBedbank, status: sevByAbs(Number(top?.bedbank_leakage_ytd ?? 0), Number(top?.revenue_ytd ?? 1)), size: 'sm' as const },
    { label: 'OTA Undercut', value: Math.round(Number(top?.ota_undercut_ytd ?? 0)), currency: ccy, footnote: fnOta, status: sevByAbs(Number(top?.ota_undercut_ytd ?? 0), Number(top?.revenue_ytd ?? 1)), size: 'sm' as const },
    { label: 'Net OTA/Web Spread', value: Math.round(netSpread), currency: ccy, footnote: fnSpread, status: sevBySpread(netSpread), size: 'sm' as const },
    { label: 'Email ADR Gap', value: Math.round(Number(top?.email_realistic_ytd ?? 0)), currency: ccy, footnote: fnEmail, status: sevByAbs(Number(top?.email_realistic_ytd ?? 0), Number(top?.revenue_ytd ?? 1)), size: 'sm' as const },
    { label: 'Phone ADR Gap', value: Math.round(Number(top?.phone_leakage_ytd ?? 0)), currency: ccy, footnote: fnPhone, status: sevByAbs(Number(top?.phone_leakage_ytd ?? 0), Number(top?.revenue_ytd ?? 1)), size: 'sm' as const },
  ];
  ...
  return (
    <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: 4, padding: '2px 0 8px', borderBottom: '1px solid var(--hairline, #E6DFCC)' }}>
      <div style={{ fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-soft, #5A5A5A)' }}>
        Headline · YTD {yr} · honest leakage + biggest single fix per channel
      </div>
