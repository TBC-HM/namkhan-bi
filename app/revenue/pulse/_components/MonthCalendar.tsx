...
// app/revenue/pulse/_components/MonthCalendar.tsx
...
// Server component prepares the dayMap; this client wrapper handles hover.
// Task #103 · 2026-05-23.
...
export interface CalendarDay {
  ...
  label?: string;       // optional badge text (e.g. "87%" or "3 ev")
  tone?: 'green' | 'amber' | 'red' | 'brass' | 'ink';
  tooltip?: string;     // hover text (multi-line OK, use \n)
}

interface Props {
  days: CalendarDay[];   // exactly the days to render, in chronological order
  variant?: 'events' | 'occ';
}
...
export default function MonthCalendar({ days, variant = 'occ' }: Props) {
  ...
  return (
    <div style={{
      ...
      {/* leading empty cells to align first day with weekday */}
      {(() => {
