// app/holding/it/cockpit/tasks/page.tsx
...
export default async function CockpitV2TasksPage({
  ...
  return (
    <div style={{ color: TOKENS.ink, fontFamily: 'var(--sans)' }}>
      ...
      >
        <FilterChip href="/holding/it/cockpit/tasks" active={filter === 'all'} label="All" count={total} />
        {Object.entries(countByStatus)
          .sort((a, b) => b[1] - a[1])
          .map(([s, n]) => (
            <FilterChip
              key={s}
              href={`/holding/it/cockpit/tasks?status=${encodeURIComponent(s)}`}
              active={filter === s}
              label={s}
              count={n}
            />
