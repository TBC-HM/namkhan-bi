...
// app/holding/it/cockpit/cost/CostView.tsx
// 60s poll on /api/holding/it/cockpit/cost. Three KPI tiles + two tables.
//
...
export function CostView({ initial }: { initial: V2CostBreakdown }) {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        setPending(true);
        const res = await fetch('/api/holding/it/cockpit/cost', { cache: 'no-store' });
        if (!res.ok) return;
              ...
              <tbody>
                {data.topTickets24h.map((t) => (
                  ...
                  >
                    <td style={td}>
                      <Link
                        href={`/holding/it/cockpit/tasks/${t.ticket_id}`}
                        style={{
                          color: TOKENS.ochre,
                          fontFamily: MONO,
                          textDecoration: 'none',
