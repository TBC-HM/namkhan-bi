...
import { GoalsView, type GoalRow, type IntakeRow } from './GoalsView';
...
export default async function CockpitGoalsPage() {
  ...
  const [{ data: goals, error: gErr }, { data: intake }] = await Promise.all([
    sb.from('v_goals')
    ...
    sb.from('v_goal_intake').select('block, question, answer, updated_at'),
  ]);
  if (gErr) {
    return <div style={{ padding: 24, color: 'var(--ink)' }}>Failed to load goals: {gErr.message}</div>;
  }
  return <GoalsView goals={(goals || []) as GoalRow[]} intake={(intake || []) as IntakeRow[]} />;
}
