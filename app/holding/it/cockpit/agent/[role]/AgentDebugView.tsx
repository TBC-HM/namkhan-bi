...
// app/holding/it/cockpit/agent/[role]/AgentDebugView.tsx
//
    ...
    try {
      const res = await fetch('/api/holding/it/cockpit/prompt', {
        ...
        body: JSON.stringify({
          ...
          prompt: draft,
          dry_run: dryRun,
          notes: `Edited via /holding/it/cockpit/agent/${bundle.role} debug surface`,
        }),
      });
  ...
  return (
    <div style={{ color: TOKENS.text }}>
      ...
      <header style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 18, flexWrap: 'wrap' }}>
        <Link href="/holding/it/cockpit/team" style={{ fontFamily: MONO, fontSize: 11, color: TOKENS.text3, textDecoration: 'none' }}>
          ← Team
        </Link>
      ...
      <div style={{ marginBottom: 18, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <Link href={`/holding/it/cockpit/agent/${bundle.role}/runs`}
              style={{
