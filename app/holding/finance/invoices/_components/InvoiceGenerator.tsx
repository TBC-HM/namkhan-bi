...
// PBS 2026-07-08: Beyond Circle invoice generator.
...
<body style="margin:0;padding:0;background:#FFFFFF;font-family:-apple-system,'SF Pro Text',Helvetica,Arial,sans-serif;color:${INK}">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:720px;margin:0 auto;background:${PAPER}">
    <tr><td style="padding:28px 32px 14px 32px;border-bottom:1px solid ${HAIRLINE}">
      ...
      <div style="font-size:12px;color:${INK_SOFT};margin-top:2px">Invoice no.: <strong style="color:${INK}">${invoiceNumber}</strong> · Issued: <strong style="color:${INK}">${new Date().toISOString().slice(0,10)}</strong>${dueAt ? ` · Due: <strong style=\"color:${INK}\">${dueAt}</strong>` : ''}</div>
    </td></tr>
        ...
        <tr>
          ...
          <td style="width:280px">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
              ...
              <tr><td style="padding:8px 10px;font-size:14px;font-weight:700;color:${INK};border-top:1px solid ${HAIRLINE}">Total</td><td style="padding:8px 10px;font-size:14px;text-align:right;font-weight:700;font-variant-numeric:tabular-nums;border-top:1px solid ${HAIRLINE};color:${PRIMARY}">${money(total)}</td></tr>
            </table>
          </td>
        </tr>
      </table>
        ...
        const r = await fetch('/api/holding/invoices/create-and-send', {
          ...
          body: JSON.stringify({
            ...
            line_items: lines.filter((l) => l.description.trim()).map((l) => ({
              description: l.description.trim(), qty: Number(l.qty), unit_price: Number(l.unit_price),
            })),
            tax_pct: Number(taxPct) || 0,
            currency,
            notes: notes.trim() || null,
            due_at: dueAt || null,
            send: !!recipientEmail.trim(),  // only actually email if we have a recipient email
          }),
        });
        if (!r.ok) throw new Error(`create failed (${r.status})`);
        const { invoice_number } = await r.json();
        setMsg(`✓ Invoice ${invoice_number} created${recipientEmail.trim() ? ' and sent' : ' (draft — no email set)'}.`);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: PAPER_SOFT, border: `1px solid ${HAIRLINE}`, borderRadius: 6 }}>
        <div style={{ fontSize: 12, color: INK_SOFT }}>Next invoice number: <strong style={{ color: INK }}>{nextNumber}</strong></div>
