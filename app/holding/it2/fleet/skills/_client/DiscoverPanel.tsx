'use client';
// DiscoverPanel -- accumulates runs, Download MD, YouTube links per card.
...
interface RunMeta {
  ...
  persisted?: boolean; saved_count?: number;
}
...
function inferFlow(name: string): string[] {
  ...
  if (n.includes('financial')||n.includes('analytic')||n.includes('narrative')) return ['GL Pull','Variance','Outlier Flag','Forecast','Report'];
  if (n.includes('thumbnail')||n.includes('content')||n.includes('video')) return ['Brief','Claude Agent','Output','Review'];
        ...
        <div style={{ padding: 14, background: '#FAFAF7', borderTop: '1px solid '+HAIR }}>
          ...
          <div style={{ marginTop: 12, display: 'flex', gap: 6, flexWrap: 'wrap' as const, alignItems: 'center' }}>
            ...
            <a href={'https://www.youtube.com/results?search_query=claude+code+'+encodeURIComponent(p.skill_name)+'+workflow'} target="_blank" rel="noopener noreferrer"
              style={{ fontSize: 10, padding: '3px 9px', background: YT+'10', color: '#8B0000', borderRadius: 4, textDecoration: 'none', fontWeight: 600 }}>▶ Claude Code pattern</a>
...
function downloadAllMd(proposals: Proposal[], userRequest: string) {
  ...
  const lines = [
    ...
    '_' + date + ' · Sources: GitHub, Reddit, Anthropic cookbook, CLAUDE.md repos, e2b awesome-ai-agents_',
    '',
    ...proposals.map(p => [
      ...
      p._reason ? '' : '',
      p._reason ? '**Scorer note:** ' + p._reason : '',
      '',
      '---',
  ...
  a.href = url;
  a.download = 'agent-flow-research-' + (userRequest||'discovery').replace(/\W+/g,'-').toLowerCase() + '-' + date + '.md';
...
export default function DiscoverPanel({ failingSkills }: { failingSkills: string[] }) {
  ...
  async function runDiscover(append = false) {
    ...
    try {
      ...
      if (j.ok) {
        ...
        setAllProposals(prev => append ? [...prev, ...newProps] : newProps);
        setLastMeta(j.metadata ?? null);
        setLastRequest(j.metadata?.user_request ?? focus);
        setRunCount(n => n + 1);
        setState('done');
      } else {
  ...
  const deduped = Array.from(new Map(allProposals.map(p => [p.skill_name, p])).values());
    ...
    <div style={{ background: WHITE, border: '2px solid '+FOREST, borderRadius: 6, overflow: 'hidden', marginBottom: 20 }}>
      ...
      <div style={{ padding: '12px 16px', background: CREAM, borderBottom: '1px solid '+HAIR, display: 'flex', gap: 10, alignItems: 'flex-end' }}>
        <div style={{ flex: 1 }}>
          ...
          <textarea value={focus} onChange={e => setFocus(e.target.value)} rows={2}
            ...
            placeholder={'"financial analyst 2-stage forecast"  ·  "replace knowledge base"  ·  "reputation review bot"  ·  "youtube thumbnail builder"'} />
        </div>
        ...
        <div style={{ padding: 16 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' as const, gap: 8 }}>
            <div style={{ fontSize: 11, color: INK_M, display: 'flex', gap: 12, flexWrap: 'wrap' as const }}>
              <span>💡 {deduped.length} proposals ({runCount} run{runCount!==1?'s':''})</span>
              {lastMeta?.repos_scanned ? <span>🔍 {lastMeta.repos_scanned} repos</span> : null}
              {lastMeta?.reddit_posts ? <span>💬 {lastMeta.reddit_posts} Reddit</span> : null}
              {lastMeta?.persisted
                ? <span style={{ color: OK }}>💾 {lastMeta.saved_count ?? 0} saved to KB</span>
                : lastMeta?.generated !== undefined ? <span style={{ color: AMBER }}>⚠ KB save failed</span> : null}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              {deduped.length > 0 && (
                <button onClick={() => downloadAllMd(deduped, lastRequest)}
                  style={{ fontSize: 11, padding: '6px 14px', background: NAVY, color: WHITE, border: 'none', borderRadius: 4, cursor: 'pointer', fontWeight: 600 }}>
                  📄 Download MD
                </button>
              )}
              {deduped.length > 0 && (
                <button onClick={() => { setAllProposals([]); setRunCount(0); setLastMeta(null); setState('idle'); }}
      ...
      {state==='idle' && deduped.length===0 && (
        <div style={{ padding: '20px 16px', fontSize: 12, color: INK_M, textAlign: 'center' as const, lineHeight: 1.9 }}>
          ...
          Use ➕ Add more to accumulate proposals across multiple runs.<br/>
          <span style={{ color: OK, fontSize: 11 }}>Each run saves individually to Supabase research KB + 📄 Download MD keeps a local copy.</span>
        </div>
      )}
    </div>
  );
