'use client';
// DiscoverPanel -- skill discovery UI with prominent prompt window + debug error display.
...
interface Proposal {
  ...
  namkhan_fit: string; effort: string; value: string; integration: string;
...
interface DiscoverMeta {
  ...
  filtered_low_quality?: number; repos_scanned?: number; curated_source?: string;
  persisted?: boolean; failing_skills?: string[]; current_skill_count?: number;
}
...
function inferFlow(p: Proposal): string[] {
  ...
  if (n.includes('retreat') && n.includes('proposal')) return ['Group Enquiry','ICP Match','Capacity','Pricing','PDF'];
  if (n.includes('concierge')) return ['Guest ICP','Activity Catalog','Season','Claude','Itinerary'];
  if (n.includes('phone') || n.includes('fo_')) return ['Caller Intent','Property Context','Claude Response','Output'];
  if (n.includes('financial') || n.includes('analytic')) return ['QB Data','Period','USALI','Analysis','Report'];
  if (n.includes('video')) return ['Brief','Script','Shotstack','Review','YouTube'];
  if (n.includes('memory') || n.includes('knowledge')) return ['Query','Semantic Search','Context Fetch','Response'];
    ...
    <div style={{ background: WHITE, border: '1px solid ' + HAIR, borderRadius: 6, overflow: 'hidden' }}>
      <div style={{ padding: '12px 14px' }}>
        <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' as const, alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span style={{ fontSize: 10, fontWeight: 700, color: match > 70 ? OK : AMBER }}>{match}%</span>
          </div>
          ...
          {p._avg && <span style={{ fontSize: 10, color: OK, padding: '2px 8px', background: OK + '15', borderRadius: 10 }}>★ {p._avg.toFixed(1)}</span>}
        </div>
      ...
      {expanded && (
        <div style={{ padding: 14, background: '#FAFAF7', borderTop: '1px solid ' + HAIR }}>
          ...
          {p._reason && <div style={{ fontSize: 11, color: INK_M, marginTop: 8, fontStyle: 'italic' }}>Scorer: {p._reason}</div>}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 12 }}>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: INK_M, textTransform: 'uppercase' as const, marginBottom: 4 }}>Namkhan fit</div>
              <div style={{ fontSize: 11, color: INK, lineHeight: 1.6 }}>{p.namkhan_fit}</div>
            </div>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: INK_M, textTransform: 'uppercase' as const, marginBottom: 4 }}>Tables & ROI</div>
...
export default function DiscoverPanel({ failingSkills }: { failingSkills: string[] }) {
  ...
  async function runDiscover() {
    setState('loading'); setProposals([]); setMeta(null); setExpanded({}); setErrDetail(null);
    try {
      const res = await fetch('/api/cockpit/skills/discover_agent_flows', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ focus: focus.trim() || 'hospitality agent flows', max_proposals: 10 }),
      });
  ...
  return (
    <div style={{ background: WHITE, border: '2px solid ' + FOREST, borderRadius: 6, overflow: 'hidden', marginBottom: 20 }}>
      ...
      <div style={{ padding: '12px 16px', background: CREAM, borderBottom: '1px solid ' + HAIR, display: 'flex', gap: 10, alignItems: 'flex-end' }}>
        <div style={{ flex: 1 }}>
          ...
          <textarea value={focus} onChange={e => setFocus(e.target.value)} rows={2}
            ...
            placeholder={'"financial analyst flows"  ·  "build retreat funnel webpage"  ·  "create videos from jpeg files"  ·  "replace knowledge base skill"'} />
        </div>
      ...
      {/* Error state — shows stage + raw preview for debugging */}
      {state === 'error' && errDetail && (
        <div style={{ padding: 16 }}>
          ...
          {errDetail.hint && <div style={{ color: INK_M, fontSize: 11, marginBottom: 10 }}>{errDetail.hint}</div>}
          {errDetail.raw && (
            <div>
              <button onClick={() => setShowRaw(r => !r)}
                style={{ fontSize: 10, padding: '3px 10px', border: '1px solid ' + HAIR, borderRadius: 3,
                  background: WHITE, color: INK_M, cursor: 'pointer', marginBottom: 6 }}>
              ...
              {showRaw && (
                <div style={{ background: CREAM, padding: '8px 12px', borderRadius: 4, fontFamily: 'monospace',
                  fontSize: 10, color: INK, whiteSpace: 'pre-wrap' as const, maxHeight: 220, overflow: 'auto',
      ...
      {state === 'done' && (
        <div style={{ padding: 16 }}>
          <div style={{ display: 'flex', gap: 14, marginBottom: 14, fontSize: 11, color: INK_M, flexWrap: 'wrap' as const }}>
            ...
            {meta?.repos_scanned ? <span>🔍 {meta.repos_scanned} repos scanned</span> : null}
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' as const, alignItems: 'center' }}>
            ...
            {([[0,'All'],[70,'>70%'],[85,'>85%'],[90,'>90%']] as const).map(([v,l]) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 12 }}>
            {filtered.length === 0
              ...
              : filtered.map((p, i) => (
                  <ProposalCard key={i} p={p} expanded={!!expanded[i]} onToggle={() => setExpanded(e => ({ ...e, [i]: !e[i] }))} />
                ))
            }
          </div>
        </div>
      ...
      {state === 'idle' && (
        <div style={{ padding: '20px 16px', fontSize: 12, color: INK_M, textAlign: 'center' as const }}>
          ...
          Scans GitHub + 8 curated frameworks (CrewAI, AutoGen, GPT Researcher, MemGPT…) and maps gaps vs {failingSkills.length > 0 ? failingSkills.length + ' failing skills' : 'skill catalog'}.<br/>
          <span style={{ color: OK, fontSize: 11 }}>Passing proposals are automatically saved to the research knowledge base.</span>
        </div>
      )}
    </div>
  );
