...
export default async function CockpitV2TasksPage({
  ...
  return (
    <div style={{ color: TOKENS.ink, fontFamily: 'var(--sans)' }}>
      ...
      >
        <FilterChip href="/holding/it2/fleet/tasks" active={filter === 'all'} label="All" count={total} />
        {Object.entries(countByStatus)
          .sort((a, b) => b[1] - a[1])
          .map(([s, n]) => (
            <FilterChip
              key={s}
              href={`/holding/it2/fleet/tasks?status=${encodeURIComponent(s)}`}
              active={filter === s}
              label={s}
              count={n}
            />
      ...
      {/* Schedule folded into Tasks (it-area-reorg-v1): the full Scheduler
          Console — pg_cron + Vercel crons + CCR agents, kill switch, audit. */}
      <section style={{ marginTop: 40, borderTop: `1px solid ${TOKENS.border}`, paddingTop: 24 }}>
        <h2 style={{ fontFamily: SERIF, fontSize: 22, margin: '0 0 14px' }}>Schedule</h2>
        <ScheduleSection />
      </section>
    </div>
  );
