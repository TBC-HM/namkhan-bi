  ...
  if (!t) {
    return (
      <div style={{ color: TOKENS.ink, fontFamily: 'var(--sans)' }}>
        <Link href="/holding/it2/fleet/tasks" style={{ color: TOKENS.ochre, fontSize: 13 }}>
          ← All tasks
        </Link>
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
        ...
        {parentTicket && (
          <Link
            href={`/holding/it2/fleet/tasks/${parentTicket}`}
            style={{ color: TOKENS.ochre, fontSize: 13, textDecoration: 'none' }}
          >
            ↑ Parent #{parentTicket}
          </Link>
        ...
        <Section title={`Children (${children.length})`}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {children.map((c) => (
              <Link
                key={c}
                href={`/holding/it2/fleet/tasks/${c}`}
                style={{
                  padding: '4px 10px',
