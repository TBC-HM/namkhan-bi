...
// app/holding/it2/fleet/team/agent/[role]/AgentDebugView.tsx
//
// Inline prompt editor + cross-reference surface. Save button posts to
// /api/holding/it/cockpit/prompt which version-bumps + writes audit log per #78.
...
export function AgentDebugView({ bundle }: { bundle: Bundle }) {
  ...
  async function save(dryRun: boolean) {
    ...
    try {
      const res = await fetch('/api/holding/it/cockpit/prompt', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          prompt: draft,
          dry_run: dryRun,
          notes: `Edited via /holding/it2/fleet/team/agent/${bundle.role} debug surface`,
        }),
      });
  ...
  return (
    <div style={{ color: TOKENS.text }}>
      ...
      <header style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 18, flexWrap: 'wrap' }}>
        <Link href="/holding/it2/fleet/team" style={{ fontFamily: MONO, fontSize: 11, color: TOKENS.text3, textDecoration: 'none' }}>
          ← Team
        </Link>
      ...
      <div style={{ marginBottom: 18, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <Link href={`/holding/it2/fleet/team/agent/${bundle.role}/runs`}
              style={{
