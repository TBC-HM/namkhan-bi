// app/holding/it2/knowledge/data/schemas/[schema]/[table]/page.tsx
//
  ...
  return (
    <div style={{ color: TOKENS.text }}>
      <header style={{ marginBottom: 18, display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
        <Link href="/holding/it2/knowledge/data/schemas" style={{ fontFamily: MONO, fontSize: 11, color: TOKENS.text3, textDecoration: 'none' }}>
          ← Schemas
        </Link>
            ...
            {d.columns.map((c, i) => (
              <tr key={i} style={{ borderBottom: `1px solid ${TOKENS.borderSoft}` }}>
                ...
                <td style={{ ...td(), color: c.fk_target ? TOKENS.forest : TOKENS.text3 }}>
                  {c.fk_target ? (
                    <Link
                      href={`/holding/it2/knowledge/data/schemas/${c.fk_target.split('.')[0]}/${c.fk_target.split('.')[1]}`}
      ...
      <div style={{ marginTop: 18, padding: 12, background: TOKENS.bgRaised, border: `1px solid ${TOKENS.borderSoft}`, borderRadius: 2 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          {d.hasPropertyId && (
            <Link href={`/holding/it2/knowledge/data/freshness`} style={btnLink(TOKENS.forest)}>Freshness matrix</Link>
          )}
          <Link href="/holding/it2/knowledge/data/schemas" style={btnLink(TOKENS.text3)}>All schemas</Link>
        </div>
      </div>
    </div>
  );
