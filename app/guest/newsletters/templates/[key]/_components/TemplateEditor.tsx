...
// Saves via fn_upsert_newsletter_template RPC (anon-callable per existing pattern).
...
const TRIGGERS = [
  ...
  { key: 'quarterly',                 label: 'Quarterly (auto — first Monday of Jan/Apr/Jul/Oct)' },
];
...
function renderMarkdownLite(md: string): string {
  // Extremely lightweight markdown → HTML for preview. Not for production render.
  let html = md
...
export default function TemplateEditor({ initial, isNew }: Props) {
  ...
  async function save() {
    ...
    try {
      ...
      if (!cleanKey) throw new Error('Template key is required (letters, numbers, underscores)');
      ...
      const { data, error } = await supabase.rpc('fn_upsert_newsletter_template', {
        p_template_key: cleanKey,
        p_property_id:  PROPERTY_ID,
        p_label:        label,
        p_description:  description,
        p_subject:      subject,
        p_category:     category,
        p_hero_image_url: heroImage || null,
        p_blocks_json:  blocksJson,
        p_body_md:      bodyMd,
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
      ...
      <div style={{ background:WHITE, border:'1px solid '+HAIR, borderRadius:6, padding:'16px 18px' }}>
        ...
        {fieldWrap('Key (URL slug)',
          <input type="text" value={templateKey} onChange={(e) => setTemplateKey(e.target.value)}
            disabled={!isNew}
            placeholder="e.g. anticipation"
            style={{ padding:'6px 10px', border:'1px solid '+HAIR, borderRadius:4, fontSize:13, color:INK }}
          />,
          isNew ? 'Lowercase letters, numbers, underscores. Cannot change after save.' : 'Locked once created.')}
        ...
        {fieldWrap('Description',
          <textarea rows={2} value={description} onChange={(e) => setDescription(e.target.value)}
            style={{ padding:'6px 10px', border:'1px solid '+HAIR, borderRadius:4, fontSize:12, color:INK, resize:'vertical' }} />,
          'One line about when this is used.')}

        {fieldWrap('Subject (email subject line)',
          <input type="text" value={subject} onChange={(e) => setSubject(e.target.value)}
            placeholder="Your Namkhan stay is coming up, {{first_name}}"
            style={{ padding:'6px 10px', border:'1px solid '+HAIR, borderRadius:4, fontSize:13, color:INK }} />,
          'Use {{first_name}}, {{days_to_arrival}}, {{booking_code}} for personalisation.')}

        {fieldWrap('Category',
          <select value={category} onChange={(e) => setCategory(e.target.value)}
            style={{ padding:'6px 10px', border:'1px solid '+HAIR, borderRadius:4, fontSize:13, color:INK, background:WHITE }}>
            {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
          </select>,
          'Transactional = triggered by a booking. Marketing = ad-hoc campaigns. Editorial = journal / storytelling.')}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:12 }}>
          ...
          {fieldWrap('Days',
            <input type="number" value={triggerDays} onChange={(e) => setTriggerDays(e.target.value)}
              disabled={triggerKind === 'manual' || triggerKind === 'quarterly'}
              placeholder="e.g. 5"
              style={{ padding:'6px 10px', border:'1px solid '+HAIR, borderRadius:4, fontSize:13, color:INK }} />)}
        </div>

        {fieldWrap('Audience hint',
          <input type="text" value={audience} onChange={(e) => setAudience(e.target.value)}
            placeholder="Guests with an upcoming arrival · pre-stay 3–7 days out"
            style={{ padding:'6px 10px', border:'1px solid '+HAIR, borderRadius:4, fontSize:13, color:INK }} />,
          'Describes the target segment. Composer will pre-fill matching filters.')}

        {fieldWrap('Hero image URL',
          <input type="text" value={heroImage} onChange={(e) => setHeroImage(e.target.value)}
            placeholder="https://..."
            style={{ padding:'6px 10px', border:'1px solid '+HAIR, borderRadius:4, fontSize:13, color:INK }} />)}

        {fieldWrap('Body (Markdown)',
          <textarea rows={12} value={bodyMd} onChange={(e) => setBodyMd(e.target.value)}
            style={{ padding:'8px 10px', border:'1px solid '+HAIR, borderRadius:4, fontSize:12, color:INK, resize:'vertical', fontFamily:'ui-monospace, SFMono-Regular, Menlo, monospace' }} />,
          'Use ## / ** ** / [link](url). Personalisation tokens: {{first_name}}, {{days_to_arrival}}, {{booking_code}}, {{weather_summary}}, {{transfer_status}}.')}

        {fieldWrap('Language / party / audience variants (advanced JSON)',
          <textarea rows={8} value={variantsRaw} onChange={(e) => setVariantsRaw(e.target.value)}
            style={{ padding:'8px 10px', border:'1px solid '+HAIR, borderRadius:4, fontSize:11, color:INK, resize:'vertical', fontFamily:'ui-monospace, SFMono-Regular, Menlo, monospace' }} />,
          'Optional. Override subject/copy for specific language/party/tier segments.')}

        <label style={{ display:'flex', gap:8, alignItems:'center', fontSize:12, color:INK_S, marginTop:6, marginBottom:12 }}>
          <input type="checkbox" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} />
          Active — allow composer to use this template
        </label>
        ...
        <div style={{ background:'#FAFAF7', border:'1px solid '+HAIR, borderRadius:6, overflow:'hidden' }}>
          ...
          <div style={{ padding:'20px 24px', background:WHITE, color:INK, fontSize:14, lineHeight:1.6 }}>
            ...
            <div style={{ borderTop:'1px solid '+HAIR, paddingTop:12 }}
              dangerouslySetInnerHTML={{ __html: renderMarkdownLite(bodyMd || '_(empty body)_') }} />
          </div>
