...
// Newsletter overview — all campaigns for the current property.
//
// Server component that queries public.v_guest_campaigns (aggregate counters
// baked into the view) and renders three groups: draft / scheduled / sent.
// PBS paper-white token per feedback_paper_white_default_for_tables.md.
...
function pctOr(n: number, d: number): string {
  if (!d) return '—';
...
export default async function NewslettersPage() {
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1200, margin: '0 auto', background: '#FAF7EE', minHeight: '100vh' }}>
      <div style={{ marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: '#5A5A5A', marginTop: 6 }}>
            {rows.length} campaign{rows.length === 1 ? '' : 's'} · property #{PROPERTY_ID}
          </div>
        </div>
      ...
      {errMsg && (
        <div style={errorBox}>Could not load campaigns: {errMsg}</div>
      )}

      <Section title="Drafts" empty="No drafts yet. Compose a newsletter from the Directory to save one here." rows={drafts} />
      ...
      {rows.length === 0 ? (
        ...
        <div style={tableWrap}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #E6DFCC', background: '#FFFFFF' }}>
                ...
                <th style={{...th, textAlign: 'right'}}>Booking hits</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.campaign_id} style={{ borderBottom: '1px solid #E6DFCC', background: '#FFFFFF' }}>
                  ...
                  <td style={tdR}>{r.send_count} {r.pending_count ? <span style={pendingChip}>{r.pending_count} pending</span> : null}{r.queued_count ? <span style={queuedChip}>{r.queued_count} queued</span> : null}{r.failed_count ? <span style={failedChip}>{r.failed_count} failed</span> : null}</td>
...
// ---- styles (hardcoded per feedback_namkhan_token_ladder_paper_warm_dark) ---
const eyebrow: CSSProperties = { fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#5A5A5A' };
const ctaButton: CSSProperties = {
  padding: '8px 16px', fontSize: 13, fontWeight: 600,
  background: '#1F3A2E', color: '#FFFFFF',
  border: '1px solid #1F3A2E', borderRadius: 4, textDecoration: 'none',
};
const sectionHeader: CSSProperties = {
  fontSize: 12, letterSpacing: '0.08em', textTransform: 'uppercase',
  color: '#5A5A5A', fontWeight: 600, marginBottom: 8,
};
const emptyState: CSSProperties = {
  padding: '16px 20px', fontSize: 13, color: '#5A5A5A',
  background: '#FFFFFF', border: '1px solid #E6DFCC', borderRadius: 6,
};
const tableWrap: CSSProperties = {
  border: '1px solid #E6DFCC', borderRadius: 6, overflow: 'hidden', background: '#FFFFFF',
};
