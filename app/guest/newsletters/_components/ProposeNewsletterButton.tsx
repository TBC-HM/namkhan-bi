...
// PBS 2026-07-22 · "✨ Propose Newsletter" — one-shot AI-drafted newsletter from a seed sentence.
//
// Flow:
//   1. Operator clicks the button → modal drawer opens.
//   2. Enters seed text + optional target date + audience.
//   3. POST /api/marketing/newsletter/propose-one — returns { subject, body_md, hero_asset_id, goal_tag }.
//   4. Modal shows preview via renderEmailFrame.
//   5. Buttons: "Save as draft" (calls fn_campaign_create_from_proposal via /save route → redirects to editor)
//              "Refine" (prompt for instruction → re-POST with instruction prepended).
...
interface Props { propertyId: number; defaultAudience?: 'b2c' | 'b2b' | 'all' }
...
export default function ProposeNewsletterButton({ propertyId, defaultAudience }: Props) {
  ...
  const previewHtml = useMemo(() => {
    ...
    return renderEmailFrame({
      heroImageUrl: prop.hero_public_url,
      heroAlt: prop.subject,
      bodyHtml: markdownToInlineHtml(prop.body_md || ''),
    ...
    startT(async () => {
      const res = await fetch('/api/marketing/newsletter/propose-one', {
        ...
        body: JSON.stringify({
          ...
          hero_asset_id: prop.hero_asset_id,
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} style={proposeCta}>✨ Propose Newsletter</button>
      ...
      {open && (
        <div role="dialog" aria-modal="true"
          ...
          <aside style={{
            width: 720, maxWidth: '96vw', height: '100vh', background: WHITE, borderLeft: `1px solid ${HAIR}`,
            overflow: 'auto', padding: 20, boxShadow: '-12px 0 32px rgba(0,0,0,0.18)',
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h4 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: INK }}>✨ Propose Newsletter</h4>
            ...
            <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '1fr 160px 120px', gap: 8, alignItems: 'end' }}>
              <label style={fieldWrap}>
                <span style={fieldLabel}>Seed — what's this newsletter about?</span>
            ...
            <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
              ...
              <span style={{ fontSize: 10, color: INK_M, alignSelf: 'center' }}>
                Applies all active email_general_rules · uses default hero + link catalog.
              </span>
            </div>
              ...
              <>
                ...
                <div style={{ marginTop: 12 }}>
                  ...
                  <div style={{ background: '#FAFAF7', border: `1px solid ${HAIR}`, borderRadius: 4, overflow: 'hidden' }}>
                    <iframe title="propose-preview" srcDoc={previewHtml} style={{ width: '100%', height: 460, border: 'none', background: '#F0EBE1', display: 'block' }} />
                  </div>
                </div>
                ...
                <div style={{ marginTop: 14, borderTop: `1px solid ${HAIR}`, paddingTop: 12 }}>
                  ...
                  <textarea rows={2} value={instr} onChange={(e) => setInstr(e.target.value)}
                            placeholder="e.g. shorter opener, punch up the CTA, swap hero for a food photo"
