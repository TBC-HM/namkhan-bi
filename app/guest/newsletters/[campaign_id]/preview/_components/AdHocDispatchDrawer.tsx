...
// PBS 2026-07-04 v2: hand-pick a guest list with 7 filter dropdowns + 4 spend checkboxes.
'use client';
...
export default function AdHocDispatchDrawer({ campaign_id, campaign_name }: Props) {
  ...
  const dispatch = async () => {
    ...
    try {
      ...
      if (j?.ok) {
        setMsg({ kind: 'ok', text: `Enqueued ${j.enqueued} · already in queue ${j.already_in_queue} · skipped ${j.skipped_no_email}. Batch sender picks up within 60s.` });
        setSelected(new Set());
      } else { setMsg({ kind: 'err', text: j?.error || 'Dispatch failed' }); }
    } catch (e) { setMsg({ kind: 'err', text: e instanceof Error ? e.message : 'Network error' }); }
    ...
    <div style={{
      ...
      <div style={{ padding: '12px 20px', borderBottom: '1px solid ' + HAIR, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={gender} onChange={(e) => setGender(e.target.value)} style={selectStyle}>
          <option value="">Any gender</option>{genders.map((g) => <option key={g} value={g}>{g}</option>)}
        </select>
        ...
        <select value={segment} onChange={(e) => setSegment(e.target.value)} style={selectStyle}>
          <option value="">All segments</option>{segments.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
        <select value={arrivalBucket} onChange={(e) => setArrivalBucket(e.target.value)} style={selectStyle}>
          <option value="">Any arrival</option>{arrivalBuckets.map((b) => <option key={b} value={b}>{b}</option>)}
        </select>
        <select value={partyType} onChange={(e) => setPartyType(e.target.value)} style={selectStyle}>
          <option value="">Any party</option>{partyTypes.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
      ...
      <div style={{ padding:'6px 20px', borderBottom:'1px solid '+HAIR, display:'flex', gap:12, flexWrap:'wrap', alignItems:'center', fontSize:11, color:INK_M }}>
        ...
        {(['restaurant','spa','activities','retail'] as const).map((k) => (
          <label key={k} style={{ display:'flex', alignItems:'center', gap:4 }}>
            <input type="checkbox" checked={!!spent[k]} onChange={(e) => setSpent((s) => ({ ...s, [k]: e.target.checked }))} /> {k}
          </label>
        ))}
