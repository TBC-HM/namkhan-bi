...
//
// PBS 2026-06-30 (v2): full-width primitives layout.
//   KPI strip on top → full-width partners table below.
//   No sub-nav strip (DashboardPage tabs cover Sales navigation).
//   No legacy hex bands — paper-white container, hairline rows.
...
export default async function B2bDmcContractsPage() {
  ...
  // Pre-compute which contract each source matches (avoids double counting)
  ...
  return (
    ...
    >
      {/* KPI tiles — full-width, six primitive tiles */}
      <B2bKpiStrip />

      {/* Partners table — full-width, paper-white container, hairline rows */}
      <div style={{ gridColumn: '1 / -1' }}>
        <Container
          title={`Partners · ${allRows.length}`}
          subtitle="Contracts on file (top) followed by uncontracted sources sending LPA business · click a row to open the channel landing"
        >
