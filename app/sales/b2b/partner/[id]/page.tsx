    ...
    <>
      ...
      <div style={{ background: 'var(--paper-warm)', border: '1px solid var(--paper-deep)', borderRadius: 8, padding: '18px 22px', margin: '8px 0 14px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14 }}>
          <div>
            ...
            <div style={{ marginTop: 8, fontSize: "var(--t-base)", color: 'var(--ink-soft)', lineHeight: 1.6 }}>
              Type: <strong>{c.partner_type}</strong> · Country: {c.country_flag ?? ''} {c.country ?? '—'} · VAT: <code style={{ fontFamily: 'var(--mono)', fontSize: "var(--t-sm)" }}>{c.vat_number ?? lorem('VAT')}</code>
              <br />
          ...
          <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
            <span style={{ background: 'var(--ink-soft)', color: 'var(--paper-warm)', padding: '6px 12px', borderRadius: 4, fontSize: "var(--t-base)" }}>Edit</span>
            <span style={{ background: 'var(--paper-warm)', border: '1px solid var(--line-soft)', color: 'var(--ink-soft)', padding: '6px 12px', borderRadius: 4, fontSize: "var(--t-base)" }}>Renew</span>
            <span style={{ background: 'var(--paper-warm)', border: '1px solid var(--line-soft)', color: 'var(--ink-soft)', padding: '6px 10px', borderRadius: 4, fontSize: "var(--t-base)" }}>⋯</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid var(--paper-deep)', marginTop: 18, marginBottom: -1 }}>
          {TABS.map((t, i) => {
            const active = i === 0;
            return (
              <span key={t} style={{ padding: '8px 14px', fontSize: "var(--t-base)", color: active ? 'var(--ink-soft)' : 'var(--ink-mute)', borderBottom: active ? '2px solid var(--brass)' : '2px solid transparent', fontWeight: active ? 600 : 400 }}>
                {i + 1}. {t}
              </span>
            );
          })}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
        <div style={{ background: 'var(--paper-warm)', border: '1px solid var(--paper-deep)', borderRadius: 8, padding: '14px 16px' }}>
          <div style={{ fontSize: "var(--t-xs)", color: 'var(--ink-mute)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Contract status</div>
          <div style={{ fontSize: "var(--t-lg)", color: 'var(--ink-soft)', lineHeight: 1.6 }}>
        ...
        <div style={{ background: 'var(--paper-warm)', border: '1px solid var(--paper-deep)', borderRadius: 8, padding: '14px 16px' }}>
          <div style={{ fontSize: "var(--t-xs)", color: 'var(--ink-mute)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Booking activity</div>
          <div style={{ fontSize: "var(--t-lg)", color: 'var(--ink-soft)', lineHeight: 1.6 }}>
            {lorem('not yet wired — needs dmc_reservation_mapping table')}<br />
            <span style={{ fontSize: "var(--t-sm)", color: 'var(--ink-mute)' }}>Will show: RNs YTD · Revenue · ADR · last booking</span>
          </div>
        </div>
        <div style={{ background: 'var(--paper-warm)', border: '1px solid var(--paper-deep)', borderRadius: 8, padding: '14px 16px' }}>
          <div style={{ fontSize: "var(--t-xs)", color: 'var(--ink-mute)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Parity health</div>
          <div style={{ fontSize: "var(--t-lg)", color: 'var(--ink-soft)', lineHeight: 1.6 }}>
            {lorem('parity scan not yet wired')}<br />
            <span style={{ fontSize: "var(--t-sm)", color: 'var(--ink-mute)' }}>Will show: open violations + last scan timestamp</span>
          </div>
        </div>
        <div style={{ background: 'var(--paper-warm)', border: '1px solid var(--paper-deep)', borderRadius: 8, padding: '14px 16px' }}>
