...
const POWERS: Record<string, string> = {
  ...
  mews_pms:             'Revenue · Pace (Donna secondary)',
  google_oauth:         'Gateway for Gmail · GBP · Calendar · YouTube',
  gmail_api:            'Newsletter · Inbox pickup · Guest comms',
  ...
  lighthouse:           'Rate shop · Compset pricing intel',
  quickbooks:           'P&L · GL · USALI · Cost governance',
  anthropic_ai:         'Felix · Lens audit · Newsletter AI · All agents',
  openai_image:         'AI Studio · Media generation',
  gemini_video:         'Video analysis (future)',
  shotstack:            'Video rendering · Social clips',
...
// Missing integrations not yet in DB
const MISSING_PROPERTY = [
  { slug: 'google_search_console', name: 'Google Search Console', powers: 'SEO module · Keyword rankings · Click data', setupUrl: 'https://search.google.com/search-console/', needed: 'Add property in Search Console → grant access to service account' },
  { slug: 'google_analytics_4', name: 'Google Analytics 4 (GA4)', powers: 'Traffic attribution · Channel mix · Conversion tracking', setupUrl: 'https://analytics.google.com/', needed: 'Create GA4 property → enable API in GCP → share view with service account' },
  { slug: 'factorial', name: 'Factorial (HR & Payroll)', powers: 'HR module · Staff records · Payroll sync · Scheduling', setupUrl: 'https://factorialhr.com/', needed: 'Generate Factorial API key → add to vault as FACTORIAL_API_KEY' },
  { slug: 'tripadvisor', name: 'TripAdvisor API', powers: 'Reputation · Review monitoring · Ranking alerts', setupUrl: 'https://www.tripadvisor.com/developers', needed: 'Apply for API access → add key to vault (future module)' },
];

const PLATFORM_SLUGS = new Set(['anthropic_ai','openai_image','gemini_video','shotstack','cloudinary','apify','nimble','github_bridge','supabase_platform','vercel']);
...
function IntCard({ r, nested }: { r: IntRow; nested?: boolean }) {
  ...
  return (
    <div style={{ background: nested ? T.bg : T.paper, border: `1px solid ${T.border}`, borderRadius: 3, padding: '11px 14px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {r.notes && <div style={{ fontSize: 10.5, color: T.grey, marginTop: 3 }}>{r.notes}</div>}
        <div style={{ display: 'flex', gap: 12, marginTop: 5, fontSize: 10, color: T.grey, flexWrap: 'wrap' }}>
          <span>Last check: {fmtWhen(r.last_check_at)}</span>
          {r.vault_secret_names && r.vault_secret_names.length > 0 && (
            <span>Secrets: {r.vault_secret_names.map(s => <code key={s} style={{ background: T.bg, borderRadius: 2, padding: '0 4px', marginRight: 4, fontFamily: 'ui-monospace,monospace' }}>{s}</code>)}</span>
          )}
          {r.website_url && <a href={r.website_url} target="_blank" rel="noreferrer" style={{ color: T.forest, textDecoration: 'underline' }}>docs</a>}
        </div>
      </div>
    </div>
  );
...
function MissingCard({ item }: { item: typeof MISSING_PROPERTY[0] }) {
  return (
    <div style={{ background: T.paper, border: `1px dashed ${T.border}`, borderRadius: 3, padding: '11px 14px', display: 'flex', gap: 12, alignItems: 'flex-start', opacity: 0.85 }}>
      <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        ...
        {count !== undefined && <span style={{ fontSize: 10, color: T.grey }}>{count} integrations</span>}
      </div>
...
export default async function DataSettingsPage({ params }: { params: { property_id: string } }) {
  ...
  const propertyRows = rows.filter(r => !PLATFORM_SLUGS.has(r.slug));
  const platformRows = rows.filter(r => PLATFORM_SLUGS.has(r.slug));
  const googleRows = rows.filter(r => GOOGLE_SLUGS.has(r.slug));
  const pmsRows = rows.filter(r => r.category === 'pms');
  const financeRows = rows.filter(r => ['quickbooks'].includes(r.slug));
  const revenueRows = rows.filter(r => ['lighthouse'].includes(r.slug));

  const okCount = rows.filter(r => (r.last_check_status ?? '').toLowerCase() === 'ok').length;
  const issueCount = rows.filter(r => !r.is_active || ['error','missing_secret','warning'].includes((r.last_check_status ?? '').toLowerCase())).length;
  ...
  const grid: React.CSSProperties = { display: 'flex', flexDirection: 'column', gap };
      ...
      <div style={{ maxWidth: 900 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 28 }}>
          {[
            ...
            { l: 'Needs attention', v: issueCount + MISSING_PROPERTY.length, color: issueCount > 0 ? T.red : T.amber },
            { l: 'Platform managed', v: platformCount, color: T.grey },
          ].map(t => (
        ...
        {/* Finance */}
        <div style={sec}>
          <SectionHeader title="Finance & Accounting" subtitle="P&L, GL, USALI cost structure, payroll sync." count={financeRows.length + 1} />
          <div style={grid}>
            {financeRows.map(r => <IntCard key={r.slug} r={r} />)}
            <MissingCard item={{ slug: 'factorial', name: 'Factorial (HR & Payroll)', powers: 'HR module · Staff records · Payroll sync · Scheduling', setupUrl: 'https://factorialhr.com/', needed: 'Generate Factorial API key → add to vault as FACTORIAL_API_KEY' }} />
          </div>
        </div>

        {/* Google Ecosystem */}
        <div style={sec}>
          <SectionHeader title="Google Ecosystem" subtitle="One OAuth connection unlocks Gmail, YouTube, Business Profile, Calendar, and more. Each service needs its API enabled in Google Cloud Console." count={googleRows.length + 2} />
          <div style={{ background: T.paper, border: `1px solid ${T.border}`, borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ padding: '10px 14px', borderBottom: `1px solid ${T.border}`, background: T.bg }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: T.ink }}>OAuth gateway — all Google services use this connection</span>
            </div>
            <div style={{ padding: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {googleRows.map(r => <IntCard key={r.slug} r={r} nested />)}
              <MissingCard item={{ slug: 'google_search_console', name: 'Google Search Console', powers: 'SEO module · Keyword rankings · Click data', setupUrl: 'https://search.google.com/search-console/', needed: 'Add property in Search Console → enable API in GCP → grant service account access' }} />
              <MissingCard item={{ slug: 'google_analytics_4', name: 'Google Analytics 4 (GA4)', powers: 'Traffic attribution · Channel mix · Conversion tracking', setupUrl: 'https://analytics.google.com/', needed: 'Create GA4 property → enable Analytics API in GCP → add service account as viewer' }} />
            </div>
          </div>
        </div>
        ...
        <div style={sec}>
          <SectionHeader title="Revenue Intelligence" subtitle="Rate shopping and compset data ingested automatically." count={revenueRows.length} />
        ...
        <div style={{ marginBottom: 6 }}>
          ...
          <p style={{ fontSize: 11.5, color: T.inkSoft, marginBottom: 16 }}>
            These are provisioned centrally and shared across all properties. No action required from you — shown here for transparency.
          </p>
        </div>

        {/* AI */}
        <div style={sec}>
          <SectionHeader title="AI & Generation" subtitle="Claude powers all agents · OpenAI for image generation · Shotstack for video." />
          <div style={grid}>{platformRows.filter(r => ['ai','media'].includes(r.category ?? '')).map(r => <IntCard key={r.slug} r={r} />)}</div>
        </div>
        ...
        <div style={{ fontSize: 10, color: T.grey, textAlign: 'center', paddingTop: 8 }}>
          Probe cadence: daily 06:00 Vientiane · {rows.length} integrations registered
        </div>
      </div>
    </DashboardPage>
  );
