...
// PBS 2026-07-08 (Helper I) — Donna Transport ops. Other Operated/Transportation slice.
...
function TopSellerTable({ rows, label = 'Description' }: { rows: TopSellerRow[]; label?: string }) {
  ...
  return (
    <div style={{ overflowX: 'auto', border: '1px solid #E0E0E0', borderRadius: 6 }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', background: '#FFFFFF' }}>
        <thead><tr><th style={th}>#</th><th style={th}>{label}</th><th style={thR}>Revenue (€)</th><th style={thR}>Units</th><th style={thR}>Last sold</th></tr></thead>
        <tbody>
          {rows.map((r, i) => (
...
export default async function DonnaTransportPage({ params, searchParams }: Props) {
  ...
  const DEPT = 'Other Operated', SUB = 'Transportation';
  ...
  const avgTicket = periodTx > 0 ? periodRev / periodTx : 0;
  ...
  const row1: KpiTileProps[] = [
    { label: 'Transport revenue', value: periodRev, currency: 'EUR', footnote: `Cloudbeds folio · ${opLabel}`, status: periodRev > 0 ? 'green' : 'grey', size: 'sm' },
    { label: 'Bookings', value: fmtInt(periodTx), footnote: `POS lines · ${opLabel}`, status: periodTx > 0 ? 'green' : 'grey', size: 'sm' },
    { label: 'Avg ticket', value: avgTicket, currency: 'EUR', footnote: `rev ÷ tx · ${opLabel}`, status: 'grey', size: 'sm' },
    { label: 'Transport / Occ Rn', value: spendPerOcc, currency: 'EUR', footnote: `spend per occupied room · ${opLabel}`, status: 'grey', size: 'sm' },
  ...
  const tabs: DashboardTab[] = OPERATIONS_SUBPAGES.map((s) => ({ key: s.href, label: s.label, href: s.href, active: s.href.endsWith('/transport') }));
  ...
  return (
    <DashboardPage title="Transport" subtitle={`Operations · Transportation · Cloudbeds folio · Donna Portals · property_id=${propertyId}`} tabs={tabs}>
      <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {catTiles.length > 0 && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10, marginTop: 6 }}>
                {catTiles.map((c) => (
                  <KpiTile key={c.category} label={c.category} value={c.revenue} currency="EUR" footnote={`${fmtPct(c.sharePct)} of transport · ${fmtInt(c.tx)} tx`} status="grey" size="sm" />
                ))}
              </div>
            </>
          )}
        ...
        <Container title="Monthly revenue" subtitle={`live from Cloudbeds folio · all-time total ${fmtEurStr(totalRev)} · ${fmtInt(totalTx)} bookings`} density="compact">
          <MonthlyBars rows={monthRows} colorRev="#8e6b9a" />
        </Container>

        <details open>
          <summary style={summaryStyle}>Top items (all-time)</summary>
          <div style={{ marginTop: 10 }}><TopSellerTable rows={top10} /></div>
        </details>
        {sellers.length > 10 && (
          <details>
            <summary style={summaryStyle}>All {sellers.length} items</summary>
