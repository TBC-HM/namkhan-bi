// app/h/[property_id]/inbox/page.tsx
// Property-scoped Gmail-style inbox. Body lifted from the legacy
// /inbox page (now a redirect) so Namkhan (260955) keeps its current
// behaviour and Donna Portals (1000001) gets a stub empty-state page
// until mail forwarding to data@thedonnaportals.com is wired up.
//
// Multi-tenant note: every inbox helper in `lib/sales` already takes a
// propertyId argument. No DB / schema changes needed — we just stop
// hardcoding 260955 inside the page and read it from the route param.
...
import {
  ...
  listInboxTabs,
  getMailboxStats,
  getThreadResponseMap,
  getThreadResponseTime,
  getInboxVolumeByDay,
  getResponseTimeHistogram,
  getTopSenders,
} from '@/lib/sales';
// Agent deliveries surface moved to /h/[property_id]/reports (PBS 2026-05-15).
import {
  VolumeByDayChart,
  MailboxVolumeChart,
  ResponseTimeChart,
} from '@/components/inbox/InboxCharts';
...
// Donna Portals has no inbound mail yet — forwarding to
// data@thedonnaportals.com lands in Phase 2. Until then render a stub.
const STUB_PROPERTIES = new Set<number>([1000001]);
...
function inboxSubpages(propertyId: number) {
  return [
    { label: 'Inquiries cockpit', href: '/sales/inquiries' },
    { label: 'Inbox',             href: `/h/${propertyId}/inbox` },
...
interface InboxPageProps {
  ...
  searchParams: { thread?: string; box?: string; dir?: string; delivery?: string };
}

// Agent-deliveries panel moved to components/inbox/AgentDeliveriesPanel.tsx
// and reached via /h/[property_id]/reports (PBS rule 2026-05-15 — sales
// inbox stays sales-only).
...
export default async function InboxPage({ params, searchParams }: InboxPageProps) {
  ...
  // Donna and other not-yet-onboarded properties render an empty email stub.
  // Agent deliveries no longer surface here — they live at /h/<id>/reports
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Email channel not connected yet
          </div>
          <div
            style={{
          ...
          <div style={{ fontSize: 'var(--t-md)', color: 'var(--ink-soft)', maxWidth: 520, margin: '0 auto' }}>
            Agent-delivered reports now live under{' '}
            <a href={`/h/${propertyId}/reports`} style={{ color: 'var(--brass)', textDecoration: 'underline' }}>
              Finance · Reports
            </a>{' '}
            (PBS rule 2026-05-15 — sales inbox stays sales-only).
            For email, set up forwarding to{' '}
            <span style={{ fontFamily: 'var(--mono)', color: 'var(--brass)' }}>
              data@thedonnaportals.com
            </span>.
          </div>
        </div>
      </Page>
  ...
  const [threads, tabs, mailboxStats, volumeByDay, respHisto, topSenders] = await Promise.all([
    listEmailThreads(propertyId, 200, filter),
    listInboxTabs(propertyId),
    getMailboxStats(propertyId),
    getInboxVolumeByDay(propertyId, 30),
    getResponseTimeHistogram(propertyId),
    getTopSenders(60, 20, propertyId).catch(() => []),
  ]);
  ...
  const [messages, threadResponseMap, selectedRespMin] = await Promise.all([
    selectedId ? getThreadMessages(selectedId, propertyId) : Promise.resolve([]),
    getThreadResponseMap(threads.map((t) => t.thread_id), propertyId),
    selectedId ? getThreadResponseTime(selectedId, propertyId) : Promise.resolve(null),
  ]);
  ...
  const basePath = `/h/${propertyId}/inbox`;
