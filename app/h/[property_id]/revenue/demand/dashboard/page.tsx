// app/h/[property_id]/revenue/demand/dashboard/page.tsx
// NEW Demand dashboard composed from _design primitives.
// Additive — leaves the legacy /revenue/demand page untouched.
// Reads existing getPaceOtb() data (mv_pace_otb).
...
export default async function DemandDashboardPage({ params, searchParams }: Props) {
  ...
  const total = rows.reduce(
    (a, r) => ({
      otb: a.otb + r.otb_roomnights,
      rev: a.rev + r.otb_revenue,
      stly: a.stly + r.stly_roomnights,
      stlyRev: a.stlyRev + r.stly_revenue,
    }),
  ...
  const signals: KpiTileProps[] = [
    { label: 'Months ahead of pace', value: monthsAhead, size: 'sm',
      footnote: 'roomnights_delta > 0', status: monthsAhead > 0 ? 'green' : 'grey' },
    { label: 'Months behind', value: monthsBehind, size: 'sm',
      footnote: 'roomnights_delta < 0', status: monthsBehind === 0 ? 'green' : 'amber' },
    { label: 'Strongest month',
      value: best ? best.ci_month : '—', size: 'sm',
      footnote: best ? `${fmtSigned(best.roomnights_delta)} RN vs STLY` : 'no data',
      status: best ? 'green' : 'grey' },
    { label: 'Softest month',
      value: worst ? worst.ci_month : '—', size: 'sm',
  ...
  const trendData = rows.map((r) => ({
    ci_month: r.ci_month,
    otb_rn:   r.otb_roomnights,
    stly_rn:  r.stly_roomnights,
  }));
  const revData = rows.map((r) => ({
    ci_month: r.ci_month,
    otb_rev:  Math.round(r.otb_revenue),
    stly_rev: Math.round(r.stly_revenue),
  }));
  const deltaData = rows.map((r) => ({
    ci_month: r.ci_month,
    rn_delta:  r.roomnights_delta,
    rev_delta: Math.round(r.revenue_delta),
  }));
  ...
  // ─── window pills ─────────────────────────────────────────────────────
  const basePath = `/h/${propertyId}/revenue/demand/dashboard`;
  const winOptions: Array<{ k: WindowKey; label: string }> = [
    { k: 'next7', label: '+7d' }, { k: 'next30', label: '+30d' },
    { k: 'next90', label: '+90d' }, { k: 'next180', label: '+180d' },
    { k: 'next365', label: '+365d' },
  ];
