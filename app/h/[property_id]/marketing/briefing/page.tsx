// app/h/[property_id]/marketing/briefing/page.tsx
// PBS 2026-09-14 — Marketing Briefing = guardrail inbox for the marketing area.
// Mirrors app/revenue/briefing/page.tsx scoped to source_area LIKE 'marketing%'.
//
// Read path : public.v_marketing_briefings + public.v_marketing_briefing_stats
// Write path: /api/marketing/briefing/decide → public.fn_briefing_decide
// Ingest    : /api/cron/briefing-evaluate?domain=mkt (cron daily + Refresh button)
...
async function loadData(pid: number): Promise<{ items: BriefingRow[]; stats: StatsRow | null }> {
  ...
  const [itemsR, statsR] = await Promise.all([
    sb.from('v_marketing_briefings')
      ...
      .limit(200),
    sb.from('v_marketing_briefing_stats').select('*').eq('property_id', pid).maybeSingle(),
  ]);
  return {
...
export default async function MarketingBriefingPage({ params }: PageProps) {
  ...
  ).map((s) => ({ key: s.href, label: s.label, href: s.href }));
  ...
  return (
    <DashboardPage
      ...
      subtitle="Every marketing guardrail that fired — triage in seconds. Accepts fire the CTA; a daily job scores whether the metric actually moved."
      tabs={deptTabs}
    >
      ...
      <div style={{ gridColumn: '1 / -1' }}>
        <Container title="How the learning loop works" subtitle="the more you triage, the sharper the signal" density="compact">
          <p style={{ margin: 0, color: '#4a4a4a', fontSize: 13, lineHeight: 1.55 }}>
            Every accept is timestamped and its metric baseline snapshotted. 3–14 days later a daily job compares the current value against that baseline in the direction the CTA proposed. Wins raise the confidence of similar guardrails (they show up higher and fire more aggressively); losses lower it (thresholds widen, that rule fires less often). Dismissals with a reason (<i>not_relevant</i>, <i>threshold_too_tight</i>, <i>false_signal</i>, <i>handled</i>) feed the same loop. This is why triaging every card matters — even a dismiss with a one-word reason trains the system.
          </p>
        </Container>
      </div>
    </DashboardPage>
