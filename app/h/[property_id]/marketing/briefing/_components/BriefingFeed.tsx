...
// app/h/[property_id]/marketing/briefing/_components/BriefingFeed.tsx
// PBS 2026-09-14 — client-side feed for the Marketing Briefing page.
// Mirrors revenue/briefing/_components/BriefingFeed.tsx with marketing
// source areas and decision endpoint.
...
const AREAS = [
  { key: 'all',                  label: 'All'           },
  { key: 'marketing.channels',   label: 'Channels'      },
  { key: 'marketing.newsletter', label: 'Newsletter'    },
  { key: 'marketing.social',     label: 'Social'        },
  { key: 'marketing.prospects',  label: 'Prospects'     },
  { key: 'marketing.rules',      label: 'Other'         },
] as const;
...
export default function BriefingFeed({ initial }: Props) {
  ...
  async function decide(row: BriefingRow, decision: 'accept' | 'dismiss' | 'snooze', reason?: string) {
    ...
    setBanner(null);
    try {
      const res = await fetch('/api/marketing/briefing/decide', {
        method: 'POST',
  ...
  return (
    <div style={{ display: 'grid', gap: 12 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
        ...
        <span style={{ ...labelStyle, marginLeft: 12 }}>Area</span>
        {AREAS.map((a) => (
          <button key={a.key} type="button" onClick={() => setArea(a.key)}
                  style={chip(area === a.key)}>{a.label}</button>
        ))}
        <span style={{ ...labelStyle, marginLeft: 12 }}>Status</span>
        {STATUSES.map((s) => (
            ...
            <div key={r.id} style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <span style={{ fontSize: 11, color: '#6b6b6b' }}>
                  {r.source_area.replace('marketing.', '')}
                </span>
              </div>
              <div>
