...
// /marketing (which computes live insights). Structure not data — the
// insights list stays empty until Donna-scoped rule wiring lands.
...
export default function MarketingHoDByProperty({ params }: { params: { property_id: string } }) {
  const pid = Number(params.property_id);
  if (!Number.isFinite(pid)) notFound();
  if (pid === NAMKHAN_PROPERTY_ID) redirect('/marketing');
  return (
    <HodLanding
      slug="marketing"
      propertyId={pid}
