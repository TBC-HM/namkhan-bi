...
// leave reportTypes empty to hide the "Build a report" option.
type ReportType = string;

interface DimGroup { key: string; label: string; options: { value: string; label: string }[] }

// Module-level dummy kept so the file compiles before the unused block is
// removed. Replaced inside the component by `reportTypeLabel`, `reportHrefBase`,
// `reportDimGroups` derived from cfg.
...
// All per-dept defaults now live in lib/dept-cfg/index.ts; the component
// reads cfg.* and derives storage keys from cfg.storageKeyPrefix. The
// constants below stay module-level because they don't vary per dept.
...
// Storage keys derived per-dept from cfg.storageKeyPrefix — see component body.
...
export default function DeptEntry({ cfg }: { cfg: DeptCfg }) {
  ...
  // Per-dept storage keys (was module-level when this was /revenue/page.tsx).
  const ATTN_KEY            = `nk.${cfg.storageKeyPrefix}.entry.attention.v2`;
  const DOCS_KEY            = `nk.${cfg.storageKeyPrefix}.entry.docs.v2`;
  const TASKS_KEY           = `nk.${cfg.storageKeyPrefix}.entry.tasks.v2`;
  const ACTIVE_PROJECT_KEY  = `nk.${cfg.storageKeyPrefix}.entry.activeProject.v1`;
  const LANG_KEY            = `nk.${cfg.storageKeyPrefix}.entry.lang.v1`;

  // Per-dept defaults + sub-page links from cfg. Aliases to keep the JSX
  // below readable without sweeping every reference.
  const DEFAULT_ATTN  = cfg.defaultAttn;
  const DEFAULT_DOCS  = cfg.defaultDocs;
  const DEFAULT_TASKS = cfg.defaultTasks;
  const QUICK_CHIPS   = cfg.quickChips;
  const DEPT_LINKS    = cfg.subPages;

  // Report-builder lookups derived from cfg.reportTypes (or undefined when
  // a dept has no report taxonomy yet — the My Docs choice modal hides the
  // "Build a report" button in that case).
  const REPORT_TYPES_LIST = cfg.reportTypes ?? [];
  const REPORT_LABEL: Record<string, string> = Object.fromEntries(
    REPORT_TYPES_LIST.map(rt => [rt.value, rt.label]),
  );
  const REPORT_HREF_BASE: Record<string, string> = Object.fromEntries(
    REPORT_TYPES_LIST.map(rt => [rt.value, rt.hrefBase]),
  );
  const REPORT_DIM_GROUPS: Record<string, DimGroup[]> = Object.fromEntries(
    REPORT_TYPES_LIST.map(rt => [rt.value, rt.dimGroups]),
  );
  ...
  useEffect(() => {
    ...
    fetch(`/api/cockpit/projects?dept=${cfg.slug}`, { cache: 'no-store' })
      .then(r => r.json())
      .then(j => {
  ...
  // Map item label keywords → the most relevant sub-route, using cfg's
  // attention routes (per dept). Falls back to cfg.defaultDrilldown.
  function attnTargetHref(label: string): string {
    const s = label.toLowerCase();
    for (const r of cfg.attentionRoutes ?? []) {
      if (s.includes(r.matcher)) return r.href;
    }
    return cfg.defaultDrilldown;
  }
    ...
    try {
      const res = await fetch('/api/cockpit/ai/assist', {
        ...
        body: JSON.stringify({
          ...
          payload: { field, name: projDraft.name, goal: projDraft.goal,dept: cfg.slug},
        }),
      });
      const j = await res.json();
        ...
        <form onSubmit={submitChat} style={{ width: '100%', maxWidth: 680 }}>
          ...
          <div style={{
            ...
            <input
              ...
              placeholder={activeProject ? `Ask ${cfg.hodName} — scoped to "${activeProject.name}"…` : cfg.chatPlaceholder}
              style={{
                flex:       1,
                background: 'transparent',
                border:     'none',
