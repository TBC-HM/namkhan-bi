'use client';
// components/docs/DocLibraryClient.tsx
// Shared document-library client: filter bar over ONE flat sortable table.
//
// PBS 2026-09-15: "this page UI makes no sense, better make dropdowns on top, this endless
// scrolling is inefficient" — the original rendered every shelf inline, so 23 shelves and 278
// files meant scrolling past everything to reach anything. Pick a shelf, a year, a file type,
// or type a name.
//
// Department-agnostic: `apiBase` points at the department's library route, which must return
// { ok, shelves, assets } in the shape below. Marketing still has its own copy at
// app/h/[property_id]/marketing/docs/LibraryClient.tsx — migrating it to this component needs a
// governance.push_shrink_waivers row (12 kB -> a few hundred bytes trips the shrink guard), so
// that is a follow-up, not a silent edit.
//
...
export default function DocLibraryClient(
  { propertyId, apiBase }: { propertyId: number; apiBase: string },
) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`${apiBase}?pid=${propertyId}`, { cache: 'no-store' });
  ...
  return (
    <div style={{ color: 'var(--ink, #1B1B1B)' }}>
      ...
      {/* Filter bar — replaces the inline shelf sections. */}
