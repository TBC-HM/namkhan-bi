...
-- Version: 20260501222828
-- Name:    fix_cloudbeds_sync_v16_invoker
-- Source:  prod kpenyneooigsyuuomgct (canonical)
-- Method:  supabase db dump --data-only --schema supabase_migrations + parse + split


-- v16: Switch from SECURITY DEFINER to SECURITY INVOKER + manual polling.
-- pg_net response visibility is blocked across SECURITY DEFINER snapshot
-- boundary. The hourly cron runs as supabase_admin which has all grants
-- needed without DEFINER.
...
BEGIN
  ...
  -- Poll loop
  resp_status := NULL; resp_err := NULL;
  FOR poll_attempt IN 1..poll_max LOOP
    PERFORM pg_sleep(1);
    SELECT status_code, error_msg INTO resp_status, resp_err
    FROM net._http_response WHERE id = rid;
    EXIT WHEN resp_status IS NOT NULL OR resp_err IS NOT NULL;
  END LOOP;

  IF resp_status IS NULL AND resp_err IS NULL THEN
    INSERT INTO sync_runs(entity, status, error_message, finished_at, metadata)
    VALUES('reservations_incremental','failed',
           'no response from pg_net within ' || poll_max || 's',
           now(), jsonb_build_object('http_request_id', rid));
    RETURN jsonb_build_object('error','no response','request_id',rid);
  END IF;
  IF resp_err IS NOT NULL THEN
    INSERT INTO sync_runs(entity, status, error_message, finished_at, metadata)
    VALUES('reservations_incremental','failed', 'pg_net: ' || resp_err, now(),
           jsonb_build_object('http_request_id', rid));
    RETURN jsonb_build_object('error', resp_err);
  END IF;
  IF resp_status >= 400 THEN
    INSERT INTO sync_runs(entity, status, error_message, finished_at, metadata)
    VALUES('reservations_incremental','failed', 'http ' || resp_status, now(),
           jsonb_build_object('http_request_id', rid, 'status', resp_status));
    RETURN jsonb_build_object('error','http error','status',resp_status);
  END IF;
  ...
  IF pages > 1 THEN
    FOR i IN 2..pages LOOP
      ...
      FOR poll_attempt IN 1..poll_max LOOP
        ...
        EXIT WHEN resp_status IS NOT NULL OR resp_err IS NOT NULL;
      END LOOP;
      IF resp_status IS NULL OR resp_status >= 400 OR resp_err IS NOT NULL THEN
        err_count := err_count + 1; CONTINUE;
      END IF;
      SELECT content::jsonb INTO resp FROM net._http_response WHERE id = rid;
...
GRANT EXECUTE ON FUNCTION public.cb_sync_recent_reservations(int) TO authenticated, anon, service_role;
