...
-- Version: 20260428145238
-- Name:    rebuild_views_using_classified_mv
...
spend AS (
  ...
  FROM mv_classified_transactions ct CROSS JOIN window_dates w
...
-- Update refresh function: classifier first, then downstream
CREATE OR REPLACE FUNCTION refresh_bi_views() RETURNS void AS $$
BEGIN
  REFRESH MATERIALIZED VIEW mv_classified_transactions;  -- non-concurrent for perf, ~10s
