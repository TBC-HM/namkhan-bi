...
-- Version: 20260501222033
-- Name:    fix_cloudbeds_sync_v14_polling
...
DECLARE
  ...
  base_url text;
  poll_attempt int;
  poll_max int := 60;       -- 60 polls × 1s = 60s ceiling
  http_timeout int := 45000; -- 45s pg_net timeout
BEGIN
  ...
  -- Poll until we have ANY response row (success OR error)
  resp := NULL; resp_status := NULL; resp_err := NULL;
  ...
  IF resp_status IS NULL THEN
    ...
    VALUES('reservations_incremental','failed',
           'pg_net timeout: ' || COALESCE(resp_err, 'no response in ' || poll_max || 's'),
           now(), jsonb_build_object('http_request_id', rid));
    RETURN jsonb_build_object('error', 'pg_net timeout', 'detail', resp_err);
  END IF;
  IF resp_status >= 400 THEN
    INSERT INTO sync_runs(entity, status, error_message, finished_at, metadata)
    VALUES('reservations_incremental','failed',
           'http ' || resp_status || ': ' || COALESCE(resp_err,''), now(),
  ...
  IF pages > 1 THEN
    FOR i IN 2..pages LOOP
      ...
      FOR poll_attempt IN 1..poll_max LOOP
        ...
        EXIT WHEN resp_status IS NOT NULL OR resp_err IS NOT NULL;
      END LOOP;
      IF resp_status IS NULL OR resp_status >= 400 THEN
...
-- Backward compatible: cb_hourly_refresh still calls without args (uses default 7d)
