...
-- Version: 20260427215110
-- Name:    add_dq_violation_dedup
-- Source:  prod kpenyneooigsyuuomgct (canonical)
-- Method:  supabase db dump --data-only --schema supabase_migrations + parse + split

-- Add fingerprint to deduplicate violations across runs
-- A "fingerprint" = (rule_id, entity_type, entity_id) — same trigger = same violation
ALTER TABLE dq.violations 
ADD COLUMN IF NOT EXISTS fingerprint text 
GENERATED ALWAYS AS (rule_id || '|' || COALESCE(entity_type,'') || '|' || COALESCE(entity_id,'')) STORED;

CREATE INDEX IF NOT EXISTS ix_dq_violations_fingerprint 
ON dq.violations(fingerprint, resolved_at) 
WHERE resolved_at IS NULL;

-- Updated run_all that resolves prior violations of same fingerprint
...
BEGIN
  ...
  -- Auto-resolve all unresolved violations from prior runs (will reappear if still failing)
  ...
  SELECT 'R-002', 'WARNING', 'pipeline',
    jsonb_build_object('hours_since_last_txn', 
      ROUND(EXTRACT(EPOCH FROM (now() - MAX(transaction_date))) / 3600, 1))
  ...
  WHERE r.status NOT IN ('canceled','no_show')
    ...
    AND r.check_in_date >= v_today - 30
  ...
  -- R-009: Future-dated transaction
  INSERT INTO dq.violations (rule_id, severity, entity_type, entity_id, details)
  ...
  -- R-010: Transactions missing item_category (aggregate count)
  INSERT INTO dq.violations (rule_id, severity, entity_type, details)
  SELECT 'R-010', 'INFO', 'pos_quality',
    jsonb_build_object('count_missing', COUNT(*))
  FROM public.transactions
  WHERE item_category_name IS NULL OR item_category_name = ''
  HAVING COUNT(*) > 50;

  -- R-013, R-014: Reconciliation gaps
  INSERT INTO dq.violations (rule_id, severity, entity_type, entity_id, details)
  SELECT 
  ...
  WHERE reconciliation_status IN ('INVESTIGATE','MINOR_GAP')
    ...
    AND NOT (period_year = EXTRACT(YEAR FROM v_today)::int 
  ...
  -- R-016: Zero occupancy on weekday past dates
  INSERT INTO dq.violations (rule_id, severity, entity_type, entity_id, details)
  ...
  -- R-021: QB account_code missing in chart
  INSERT INTO dq.violations (rule_id, severity, entity_type, entity_id, details)
  ...
  WHERE status IN ('pending','processing','firing') 
    AND created_at < now() - interval '1 hour'
  LIMIT 100;

  -- R-024: Sync error rate > 10%
  INSERT INTO dq.violations (rule_id, severity, entity_type, details)
  ...
  FROM (
    SELECT 
      COUNT(*) FILTER (WHERE status = 'failed' OR error_msg IS NOT NULL) AS error_count,
      COUNT(*) AS total
    FROM public.sync_request_queue
  ...
  SELECT 'R-025', 'WARNING', 'fx_rates',
    jsonb_build_object('latest_rate_date', MAX(rate_date), 
                       'days_old', CURRENT_DATE - MAX(rate_date))
  FROM gl.fx_rates
  HAVING MAX(rate_date) < CURRENT_DATE - 7;

  -- R-027: Budget vs Actual variance > 50%
  INSERT INTO dq.violations (rule_id, severity, entity_type, entity_id, details)
  SELECT 'R-027', 'INFO', 'budget_variance', pl_section,
    jsonb_build_object('section', pl_section, 'budget', budget_usd, 
                       'actual', actual_usd, 'var_pct', var_pct)
  FROM plan.snapshot(EXTRACT(YEAR FROM CURRENT_DATE)::int)
  WHERE ABS(COALESCE(var_pct, 0)) > 50
    AND budget_usd > 1000;

  -- H-001: Day-use reservation
  INSERT INTO dq.violations (rule_id, severity, entity_type, entity_id, details)
  ...
  FROM public.reservations
  WHERE check_in_date = check_out_date
    AND check_in_date >= v_today - 90
    AND status NOT IN ('canceled','no_show')
  LIMIT 100;

  -- H-002: Channel concentration > 70%
  INSERT INTO dq.violations (rule_id, severity, entity_type, details)
  ...
  FROM (
    SELECT 
      COUNT(*) FILTER (WHERE status = 'canceled' OR is_cancelled = true) AS canceled,
      COUNT(*) AS total
    FROM public.reservations
  ...
  WHERE v.detected_at >= v_run_started
