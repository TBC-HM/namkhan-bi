...
-- Version: 20260504004721
-- Name:    v_catalog_dirty_exclude_rates_2026_05_04
-- Source:  prod kpenyneooigsyuuomgct (canonical)
-- Method:  supabase db dump --data-only --schema supabase_migrations + parse + split


-- Exclude room rates and credit-card commission from the cleanup queue.
-- Room rates legitimately vary by night (yield management) so multi-price is by-design.
-- Credit Card Commission is a payment-processor fee, not a product.
...
WITH base AS (
  ...
  WHERE t.property_id = 260955
    ...
    AND COALESCE(t.category,'') NOT IN ('tax','fee','payment','rate')
    AND t.description NOT ILIKE 'Room rate%'
    AND t.description NOT ILIKE '%Credit Card Commission%'
), agg AS (
  SELECT
    usali_dept,
    usali_subdept,
