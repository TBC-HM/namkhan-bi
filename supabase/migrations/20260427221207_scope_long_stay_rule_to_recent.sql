...
-- Version: 20260427221207
-- Name:    scope_long_stay_rule_to_recent
-- Source:  prod kpenyneooigsyuuomgct (canonical)
-- Method:  supabase db dump --data-only --schema supabase_migrations + parse + split

-- Update dq.run_all so R-008 only fires on recent (last 365 days) long stays
-- Just patching the relevant section by recreating the whole function
-- The change: add `AND check_in_date >= CURRENT_DATE - 365` to R-008 query
...
BEGIN
  ...
  -- R-008: Stay > 30 nights, RECENT ONLY (last 365 days)
  INSERT INTO dq.violations (rule_id, severity, entity_type, entity_id, details)
  ...
  WHERE (check_out_date - check_in_date) > 30
    ...
    AND check_in_date >= v_today - 365   -- only recent
  LIMIT 100;
