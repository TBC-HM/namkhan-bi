-- ============================================================================
-- Migration: 09_dq_count_variance_trigger  (Phase 2.5)
-- Version:   20260502230018
-- Date:      2026-05-02
-- ----------------------------------------------------------------------------
-- Spec: stocktake variance >5% OR >$50 must auto-generate a DQ finding.
-- Routes to existing gl.dq_findings (the canonical DQ table).
-- Conventions match the DQ engine in production:
--   rule_code = DQ-INV-COUNT-VARIANCE
--   severity  = critical (>$500 or >25%) | high (>$50 or >5%) | med (boundary)
--   ref_type  = inv_count_line
--   ref_id    = count_line_id (text)
--   fingerprint = INV-COUNT-VARIANCE:<count_line_id> — stable, dedupes via UPSERT
-- ============================================================================
