-- ============================================================================
-- Migration: 01_suppliers_schema  (Phase 2.5 foundation)
-- Version:   20260502230010
-- Date:      2026-05-02
-- ----------------------------------------------------------------------------
-- Strategic vendor master. Sits ABOVE qb.vendors (which is the QB-mirrored
-- transactional list). qb_vendor_ref optionally links the two.
--
-- Why a separate schema: qb.* is for QuickBooks-sourced data. Strategic
-- supplier attributes (lead time, sustainability score, distance for local
-- sourcing %, alternate supplier, payment terms) are operational, not
-- accounting, and should not pollute qb.*.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS suppliers.suppliers (
  ...
  -- Geography (drives local-sourcing % metric)
...
-- ----------------------------------------------------------------------------
-- 3. suppliers.price_history
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS suppliers.price_history (
  price_id             BIGSERIAL PRIMARY KEY,
  supplier_id          UUID NOT NULL REFERENCES suppliers.suppliers(supplier_id) ON DELETE CASCADE,
  inv_item_id          UUID,                            -- soft FK to inv.items (FK added in followup)
  inv_sku              TEXT,                            -- denormalized for easy join even if item deleted
...
-- ----------------------------------------------------------------------------
-- 4. suppliers.alternates  (preferred → fallback supplier mapping)
-- ----------------------------------------------------------------------------
...
-- supplier overview with rollups
...
-- local sourcing % over the trailing 12 months (computed once inv.movements exists)
-- Built as a placeholder that returns NULL until inv schema lands.
...
WHERE status = 'active';
COMMENT ON VIEW suppliers.v_local_sourcing_pct IS 'Initial cut: %% of active suppliers that are local. Will be replaced with $-weighted version once inv.movements has data.';
