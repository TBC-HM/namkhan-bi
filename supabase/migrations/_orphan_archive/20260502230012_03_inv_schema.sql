-- ============================================================================
-- Migration: 03_inv_schema  (Inventory — par stock + movements + counts)
-- Version:   20260502230012
-- Date:      2026-05-02
-- DEPENDS ON: suppliers (FK on items.preferred_supplier_id, movements.supplier_id)
-- ============================================================================
-- Implements pages: /ops/inventory, /ops/inventory/items/[id], /ops/inventory/shop,
--                   /ops/inventory/counts (and underpins /ops/inventory/orders receipts).
-- All views the spec calls live in this migration:
--   v_inv_stock_on_hand, v_inv_par_status, v_inv_slow_movers,
--   v_inv_heatmap_health, v_inv_usage_trend, v_inv_days_of_cover, v_inv_expiring_soon
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS inv.locations (
  ...
  code                 TEXT NOT NULL UNIQUE,
  location_name        TEXT NOT NULL,                   -- spec uses 'location_name' for joins
  ...
  responsible_dept     TEXT,                            -- e.g. 'fb', 'hk', 'spa'
  is_active            BOOLEAN NOT NULL DEFAULT true,
  notes                TEXT,
  created_at           TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at           TIMESTAMPTZ NOT NULL DEFAULT now()
...
-- ----------------------------------------------------------------------------
-- 4. inv.items  (SKU master)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS inv.items (
  item_id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  sku                  TEXT NOT NULL UNIQUE,
  item_name            TEXT NOT NULL,                   -- spec uses item_name
  description          TEXT,
  ...
  -- Reorder / par at item level (per-location overrides in inv.par_levels)
  ...
  -- Catalog approval (Page 6 — catalog admin)
...
-- ----------------------------------------------------------------------------
-- 6. inv.stock_balance  (current on-hand snapshot per item × location)
-- ----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS inv.movements (
  ...
  quantity             NUMERIC(12,3) NOT NULL,          -- signed: + into location, − out
  ...
  -- Counterparties
  vendor_id            UUID REFERENCES suppliers.suppliers(supplier_id) ON DELETE SET NULL,  -- for receives
  counterparty_location_id BIGINT REFERENCES inv.locations(location_id) ON DELETE SET NULL,  -- for transfers
  -- Reference (PO, QB bill, count, manual)
  reference_type       TEXT,                            -- 'po','qb_bill','count','transfer','manual'
  reference_id         TEXT,
  -- Batch / expiry (for perishables)
  batch_code           TEXT,
  expiry_date          DATE,
  movement_date        DATE NOT NULL DEFAULT CURRENT_DATE,   -- spec uses movement_date
...
-- ----------------------------------------------------------------------------
-- 9. inv.count_lines (one per item per count)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 10. inv.photos (Page 3 Shop UI thumbnails + Page 2 Item detail gallery)
-- ----------------------------------------------------------------------------
...
-- ============================================================================
-- VIEWS  (the 7 the frontend spec calls)
-- ============================================================================

-- v_inv_stock_on_hand: total qty + value across locations per item
...
-- v_inv_par_status: classification per item × location vs par
...
-- v_inv_usage_trend: weekly consumption per item over last 12 weeks
CREATE OR REPLACE VIEW inv.v_inv_usage_trend AS
WITH w AS (
  SELECT
    item_id,
    date_trunc('week', movement_date)::date AS week_start,
    -- consumption = absolute value of outgoing (issue/consume/waste/transfer_out)
    SUM(CASE WHEN movement_type IN ('issue','consume','waste','transfer_out')
             THEN ABS(quantity) ELSE 0 END) AS units_consumed,
...
-- v_inv_slow_movers: items with low or zero consumption in last 90 days
CREATE OR REPLACE VIEW inv.v_inv_slow_movers AS
WITH consumption AS (
  SELECT
    item_id,
    SUM(CASE WHEN movement_type IN ('issue','consume','waste','transfer_out')
             THEN ABS(quantity) ELSE 0 END) AS units_90d
  FROM inv.movements
...
WHERE i.is_active
  AND COALESCE(con.units_90d, 0) <= 5         -- threshold: ≤5 units in 90d = slow
  AND COALESCE(soh.total_on_hand, 0) > 0;     -- only flag if we have stock sitting

-- v_inv_days_of_cover: how long current stock lasts at recent burn
CREATE OR REPLACE VIEW inv.v_inv_days_of_cover AS
WITH burn AS (
  SELECT
    item_id,
    SUM(CASE WHEN movement_type IN ('issue','consume','waste','transfer_out')
             THEN ABS(quantity) ELSE 0 END) / 30.0 AS units_per_day_30d
  FROM inv.movements
...
SELECT
  ...
  CASE
    WHEN COALESCE(b.units_per_day_30d, 0) = 0 THEN NULL
    ELSE ROUND(COALESCE(soh.total_on_hand, 0) / b.units_per_day_30d, 1)
  END AS days_of_cover,
  i.reorder_point,
  CASE
    WHEN COALESCE(b.units_per_day_30d, 0) = 0 OR i.reorder_point IS NULL THEN NULL
    ELSE ROUND((COALESCE(soh.total_on_hand, 0) - i.reorder_point) / NULLIF(b.units_per_day_30d, 0), 1)
...
-- v_inv_expiring_soon: receive movements with expiry inside 60d window where stock still on hand
CREATE OR REPLACE VIEW inv.v_inv_expiring_soon AS
SELECT
  m.movement_id   AS batch_movement_id,
  m.item_id,
  i.sku,
  i.item_name,
  m.location_id,
...
-- v_inv_heatmap_health: location × category status grid for Page 1
...
-- ============================================================================
-- SEED  (10 categories + 13 UoM + 5 default locations)
-- ============================================================================
