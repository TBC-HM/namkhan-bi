-- marketing.classify_tier — rule-based tier refinement on top of AI tagger.
-- APPLIED 2026-05-05 via Supabase MCP.
...
BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM marketing.media_tags mt JOIN marketing.media_taxonomy tx ON tx.tag_id = mt.tag_id WHERE mt.asset_id = p_asset_id AND tx.tag_slug = 'no_people') INTO v_has_no_people;
  SELECT EXISTS(SELECT 1 FROM marketing.media_tags mt JOIN marketing.media_taxonomy tx ON tx.tag_id = mt.tag_id WHERE mt.asset_id = p_asset_id AND tx.category = 'room_type') INTO v_has_room_subject;
  SELECT EXISTS(SELECT 1 FROM marketing.media_tags mt JOIN marketing.media_taxonomy tx ON tx.tag_id = mt.tag_id WHERE mt.asset_id = p_asset_id AND tx.tag_slug IN ('wide_environmental','editorial','drone_aerial_style')) INTO v_has_environment_style;
  SELECT EXISTS(SELECT 1 FROM marketing.media_tags mt JOIN marketing.media_taxonomy tx ON tx.tag_id = mt.tag_id WHERE mt.asset_id = p_asset_id AND tx.tag_slug IN ('golden_hour_morning','golden_hour_evening','sunrise','sunset','blue_hour')) INTO v_has_golden_hour;
...
-- Cron job 45: tag-media every 1 minute (idempotent)
DO $$ BEGIN PERFORM cron.unschedule('tag-media'); EXCEPTION WHEN OTHERS THEN NULL; END $$;
SELECT cron.schedule(
  'tag-media',
  '* * * * *',
  $cron$SELECT net.http_post(
    url:='https://kpenyneooigsyuuomgct.supabase.co/functions/v1/tag-media',
