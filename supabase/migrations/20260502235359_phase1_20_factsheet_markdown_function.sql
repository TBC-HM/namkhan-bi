...
-- Version: 20260502235359
-- Name:    phase1_20_factsheet_markdown_function
-- Source:  prod kpenyneooigsyuuomgct (canonical)
-- Method:  supabase db dump --data-only --schema supabase_migrations + parse + split

-- ============================================================================
-- f_factsheet_markdown(): renders the consolidated factsheet as markdown
-- For: AI agent system prompts, sales proposal headers, internal briefs
-- ============================================================================
...
BEGIN
  ...
  out := out || '_Auto-generated from marketing.v_namkhan_factsheet · Last profile update: '||(v->>'profile_updated_at')||'_' || E'\n\n';
  ...
  IF v->'identity'->'usps' IS NOT NULL THEN
    FOR r IN SELECT jsonb_array_elements(v->'identity'->'usps') LOOP
      out := out || '- ' || (r#>>'{}') || E'\n';
    END LOOP;
  END IF;
  out := out || E'\n';
  ...
  IF v->'rooms' IS NOT NULL THEN
    FOR r IN SELECT * FROM jsonb_array_elements(v->'rooms') LOOP
      out := out || '| '||COALESCE(r->>'name','')
                 || ' | '||COALESCE(r->>'size_sqm','')||' m²'||COALESCE(' + '||(r->>'garden_sqm')||' m² garden','')
                 || ' | '||COALESCE(r->>'units','')
  ...
  IF v->'facilities' IS NOT NULL THEN
    FOR r IN SELECT key, value FROM jsonb_each(v->'facilities') LOOP
      out := out || '- **'||initcap(r->>'key')||':** '||
        (SELECT string_agg(item->>'name', ' · ')
         FROM jsonb_array_elements(r->'value') AS item) || E'\n';
    END LOOP;
  END IF;
  out := out || E'\n';
  ...
  IF v->'activities' IS NOT NULL THEN
    FOR r IN SELECT key, value FROM jsonb_each(v->'activities') LOOP
      out := out || '- **'||initcap(r->>'key')||':** '||
        (SELECT string_agg(item->>'name', ' · ')
         FROM jsonb_array_elements(r->'value') AS item) || E'\n';
    END LOOP;
...
COMMENT ON FUNCTION marketing.f_factsheet_markdown IS
  'Renders consolidated factsheet as markdown. Use for AI agent prompts, proposal headers, internal briefs.';

GRANT EXECUTE ON FUNCTION marketing.f_factsheet_markdown TO authenticated;