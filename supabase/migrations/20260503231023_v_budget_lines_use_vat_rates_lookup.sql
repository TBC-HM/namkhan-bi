...
-- Version: 20260503231023
-- Name:    v_budget_lines_use_vat_rates_lookup
...
SELECT
  ...
  -- Net amount = gross / (1 + vat_rate/100). vat_rate sourced from gl.vat_rates,
  -- defaulting to 0 when subcat not present (no strip).
  ROUND(
...
LEFT JOIN gl.vat_rates v
  ON v.usali_subcategory = a.usali_subcategory AND v.applies_to IN ('budget','both')
...
'Budget 2026 v1 lines for /finance/pnl. VAT rates sourced from gl.vat_rates lookup (editable via /settings/vat-rates). Subcats with applies_to in (budget, both) get net = gross / (1 + rate/100). Subcats not present in lookup or applies_to=none stay gross.';
