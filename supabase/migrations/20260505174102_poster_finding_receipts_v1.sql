...
-- Version: 20260505174102
-- Name:    poster_finding_receipts_v1
-- Source:  prod kpenyneooigsyuuomgct (canonical)
-- Method:  supabase db dump --data-only --schema supabase_migrations + parse + split

-- Receipt-level drilldown: actual rows per finding kind, enriched with the
-- matched Cloudbeds reservation (when one was found by the matcher).
CREATE OR REPLACE FUNCTION public.poster_finding_receipts(p_kind text, p_limit integer DEFAULT 200)
RETURNS TABLE (
  receipt_id        bigint,
  open_at           timestamptz,
  ...
  status            text,
  payment_method    text,
  -- enrichment from matched reservation
...
-- Smoke test
SELECT receipt_id, order_source, poster_client, cb_guest_name, cb_room_type, in_house_at_close
FROM public.poster_finding_receipts('open_receipts', 5);