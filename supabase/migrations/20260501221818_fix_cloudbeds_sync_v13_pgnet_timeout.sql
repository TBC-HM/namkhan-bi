...
-- Version: 20260501221818
-- Name:    fix_cloudbeds_sync_v13_pgnet_timeout
-- Source:  prod kpenyneooigsyuuomgct (canonical)
-- Method:  supabase db dump --data-only --schema supabase_migrations + parse + split


-- =====================================================================
-- v13: Fix pg_net 5-second default timeout. Cloudbeds endpoints with
--      includeGuestsDetails=true take 4.6-15s. Pass timeout_milliseconds.
--      Also fix sync_runs.status: must be 'failed' not 'error' (check constraint).
-- =====================================================================
...
DECLARE
  ...
  base_url text;
  poll_attempt int;
  poll_max int := 30;       -- 30 polls × 1s
  http_timeout int := 30000; -- 30s per call (Cloudbeds can be slow)
BEGIN
  ...
  FOR poll_attempt IN 1..poll_max LOOP
    PERFORM pg_sleep(1);
    SELECT
      CASE WHEN status_code IS NULL THEN NULL ELSE content::jsonb END,
      status_code
    INTO resp, resp_status
    FROM net._http_response WHERE id = rid;
    EXIT WHEN resp IS NOT NULL OR resp_status IS NOT NULL;
  END LOOP;

  IF resp IS NULL THEN
    INSERT INTO sync_runs(entity, status, error_message, finished_at, metadata)
    VALUES('reservations_incremental','failed',
           'first call timed out after ' || poll_max || 's',now(),
           jsonb_build_object('http_request_id', rid, 'http_status', resp_status));
    RETURN jsonb_build_object('error','first call timed out','http_status',resp_status);
  END IF;
  ...
  IF pages > 1 THEN
    FOR i IN 2..pages LOOP
      ...
      FOR poll_attempt IN 1..poll_max LOOP
        PERFORM pg_sleep(1);
        SELECT CASE WHEN status_code IS NULL THEN NULL ELSE content::jsonb END
        INTO resp FROM net._http_response WHERE id = rid;
        EXIT WHEN resp IS NOT NULL;
      END LOOP;
      IF resp IS NULL THEN err_count := err_count + 1; CONTINUE; END IF;
