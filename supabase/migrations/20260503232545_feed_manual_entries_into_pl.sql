...
-- Version: 20260503232545
-- Name:    feed_manual_entries_into_pl
-- Source:  prod kpenyneooigsyuuomgct (canonical)
-- Method:  supabase db dump --data-only --schema supabase_migrations + parse + split


-- Combined PL source: matview rows (QB-derived) + manual entries.
-- Used by every downstream USALI summary so manual lines automatically appear in P&L.
CREATE OR REPLACE VIEW gl.v_pl_monthly_combined AS
SELECT
  period_yyyymm,
  fiscal_year,
  usali_section,
...
'QB matview actuals + gl.manual_entries combined. Source for v_usali_dept_summary, v_usali_house_summary, v_usali_undistributed, v_budget_vs_actual, v_scenario_stack so manual entries flow into P&L without ETL.';

-- Drop dependent views with CASCADE to allow recreation
...
-- Rebuild v_usali_dept_summary against combined view
...
-- Rebuild v_usali_house_summary against combined view
...
-- Rebuild v_actuals_with_manual (now redundant with v_pl_monthly_combined but kept for API compat)
...
-- Rebuild v_budget_vs_actual against combined
...
-- Rebuild v_scenario_stack against combined
